...
from agentinsight._client.get_client import get_client
from agentinsight._client.propagation import _detach_context_token_safely
...
from agentinsight.langchain.utils import _extract_model_name
from agentinsight.logger import agentinsight_logger
from agentinsight.types import TraceContext
...
try:
    ...
    raise ModuleNotFoundError(
        "Please install langchain to use the AgentInsight langchain integration: 'pip install langchain'"
    )
...
class LangchainCallbackHandler(LangchainBaseCallbackHandler):
    def __init__(
        self,
        *,
        client: Optional[AgentInsight] = None,
        ...
        Args:
            client: Optional AgentInsight client instance. If not provided, will create one from environment variables.
            public_key: Optional AgentInsight public key. If not provided, will use the default client configuration.
        ...
        Example:
            ...
            ```
        """
        self._agentinsight_client = client or get_client(public_key=public_key)
        self._runs: Dict[
            UUID,
            Union[
                AgentInsightSpan,
    ...
    def on_llm_new_token(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            and isinstance(self._runs[run_id], AgentInsightGeneration)
            and run_id not in self._updated_completion_start_time_memo
        ):
            current_generation = cast(AgentInsightGeneration, self._runs[run_id])
    ...
    def _get_observation_type_from_serialized(
        ...
        """Determine AgentInsight observation type from LangChain component.
        ...
        Returns:
            The appropriate AgentInsight observation type string
        """
    ...
    def on_retriever_error(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            agentinsight_logger.exception(e)
        finally:
            if parent_run_id is None:
                self._reset(run_id)

    def _parse_agentinsight_trace_attributes(
        ...
        if "agentinsight_session_id" in metadata and isinstance(
            metadata["agentinsight_session_id"], str
        ):
            attributes["session_id"] = metadata["agentinsight_session_id"]
        elif "langfuse_session_id" in metadata and isinstance(
            metadata["langfuse_session_id"], str
        ):
            attributes["session_id"] = metadata["langfuse_session_id"]

        if "agentinsight_user_id" in metadata and isinstance(
            metadata["agentinsight_user_id"], str
        ):
            attributes["user_id"] = metadata["agentinsight_user_id"]
        elif "langfuse_user_id" in metadata and isinstance(
            metadata["langfuse_user_id"], str
        ):
            attributes["user_id"] = metadata["langfuse_user_id"]

        if "agentinsight_trace_name" in metadata and isinstance(
            metadata["agentinsight_trace_name"], str
        ):
            attributes["trace_name"] = metadata["agentinsight_trace_name"]
        elif "langfuse_trace_name" in metadata and isinstance(
        ...
        if (
            tags is not None
            or (
                "agentinsight_tags" in metadata
                and isinstance(metadata["agentinsight_tags"], list)
            )
            or (
                "agentinsight_tags" in metadata
                and isinstance(metadata["agentinsight_tags"], list)
            ...
            agentinsight_tags = (
                metadata["agentinsight_tags"]
                if "agentinsight_tags" in metadata
                and isinstance(metadata["agentinsight_tags"], list)
                else []
            )
            agentinsight_tags = (
            ...
            merged_tags = list(
                set(agentinsight_tags) | set(agentinsight_tags) | set(tags or [])
            )
            attributes["tags"] = [str(tag) for tag in set(merged_tags)]

        attributes["metadata"] = _strip_agentinsight_keys_from_dict(metadata, False)
    ...
    def _get_langchain_observation_metadata(
        ...
        *,
        ...
        metadata: Optional[Dict[str, Any]] = None,
        keep_agentinsight_trace_attributes: bool = False,
    ) -> Optional[Dict[str, Any]]:
        observation_metadata = self.__join_tags_and_metadata(
            tags=tags,
            metadata=metadata,
            keep_agentinsight_trace_attributes=keep_agentinsight_trace_attributes,
        )
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self._register_agentinsight_prompt(
            ...
            if parent_run_id is None:
                parsed_trace_attributes = self._parse_agentinsight_trace_attributes(
            ...
            if isinstance(obs, AgentInsight):
    ...
    def _register_agentinsight_prompt(
        self,
        *,
        run_id: UUID,
        parent_run_id: Optional[UUID],
        ...
        """We need to register any passed AgentInsight prompt to the parent_run_id so that we can link following generations with that prompt.
        ...
        agentinsight_prompt = (
            metadata and metadata.get("agentinsight_prompt", None)
        ) or (metadata and metadata.get("agentinsight_prompt", None))

        if agentinsight_prompt:
            self._prompt_to_parent_run_map[parent_run_id] = agentinsight_prompt
    ...
    def _deregister_agentinsight_prompt(self, run_id: Optional[UUID]) -> None:
    ...
    def _get_parent_observation(
        ...
        return self._agentinsight_client
        ...
        try:
            ...
            if agent_run is not None:
                agent_run._otel_span.set_attribute(
                    AgentInsightOtelSpanAttributes.OBSERVATION_TYPE, "agent"
                )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if span is not None:
                ...
                self._deregister_agentinsight_prompt(run_id)
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if isinstance(parent_observation, AgentInsight):
        ...
        try:
            ...
            while current_parent_run_id is not None:
                ...
                if registered_prompt:
                    self._deregister_agentinsight_prompt(current_parent_run_id)
                    break
                else:
            ...
            content = {
                ...
                "metadata": self._get_langchain_observation_metadata(
                    ...
                    keep_agentinsight_trace_attributes=True
                    if parent_run_id is None
                    else False,
                ),
                "model": model_name,
    ...
    def __join_tags_and_metadata(
        ...
        return (
            _strip_agentinsight_keys_from_dict(
                final_dict, keep_agentinsight_trace_attributes
            )
            if final_dict != {}
            else None
        )
...
def _parse_usage_model(usage: Union[pydantic.BaseModel, dict]) -> Any:
    ...
    for model_key, agentinsight_key in conversion_list:
...
def _strip_agentinsight_keys_from_dict(
    metadata: Optional[Dict[str, Any]], keep_agentinsight_trace_attributes: bool
) -> Any:
    if metadata is None or not isinstance(metadata, dict):
        return metadata

    agentinsight_metadata_keys = [
        "agentinsight_prompt",
        "agentinsight_prompt",
    ]

    agentinsight_trace_attribute_keys = [
        "langfuse_session_id",
        "langfuse_user_id",
        "agentinsight_tags",
        "langfuse_trace_name",
        "agentinsight_session_id",
        "agentinsight_user_id",
        "agentinsight_tags",
        "agentinsight_trace_name",
    ]

    metadata_copy = metadata.copy()

    for key in agentinsight_metadata_keys:
        metadata_copy.pop(key, None)

    if not keep_agentinsight_trace_attributes:
        for key in agentinsight_trace_attribute_keys:
            metadata_copy.pop(key, None)
