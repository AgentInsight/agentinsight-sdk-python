...
class TextPromptClient(BasePromptClient):
    ...
    def get_langchain_prompt(self, **kwargs: Union[str, Any]) -> str:
        """Convert AgentInsight prompt into string compatible with Langchain PromptTemplate.

        This method adapts the mustache-style double curly braces {{variable}} used in AgentInsight
...
class ChatPromptClient(BasePromptClient):
    ...
    def compile(
        ...
        **kwargs: Union[str, Any],
            ...
            if chat_message["type"] == "message":
                ...
                if placeholder_name in kwargs:
                    ...
                    if isinstance(placeholder_value, list):
                        for msg in placeholder_value:
                            if isinstance(msg, dict):
                                ...
                                agentinsight_logger.warning(
                                    no_role_content_in_placeholder
                                )
                    else:
                        compiled_messages.append(
                            ChatMessageDict(
                        ...
                        agentinsight_logger.warning(placeholder_not_a_list)
                else:
        ...
        if unresolved_placeholders:
            ...
            agentinsight_logger.warning(unresolved_placeholders_message)
    ...
    def get_langchain_prompt(
        ...
        """Convert AgentInsight prompt into string compatible with Langchain ChatPromptTemplate.

        It specifically adapts the mustache-style double curly braces {{variable}} used in AgentInsight
        to the single curly brace {variable} format expected by Langchain.
