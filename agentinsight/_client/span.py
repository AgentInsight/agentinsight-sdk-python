"""OTEL span wrapper for AgentInsight.

This module defines custom span classes that extend OpenTelemetry spans with
AgentInsight-specific functionality. These wrapper classes provide methods for
creating, updating, and scoring various types of spans used in AI application tracing.

Classes:
- AgentInsightObservationWrapper: Abstract base class for all AgentInsight spans
- AgentInsightSpan: Implementation for general-purpose spans
- AgentInsightGeneration: Specialized span implementation for LLM generations

All span classes provide methods for media processing, attribute management,
and scoring integration specific to AgentInsight's observability platform.
"""
...
from agentinsight.api import MapValue, ScoreDataType
...
_OBSERVATION_CLASS_MAP: Dict[str, Type["AgentInsightObservationWrapper"]] = {}


class AgentInsightObservationWrapper:
    """Abstract base class for all AgentInsight span types.

    This class provides common functionality for all AgentInsight span types, including
    media processing, attribute management, and scoring. It wraps an OpenTelemetry
    span and extends it with AgentInsight-specific features.

    Attributes:
        _otel_span: The underlying OpenTelemetry span
        _client: Reference to the parent AgentInsight client
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new AgentInsight span wrapper.

        Args:
            otel_span: The OpenTelemetry span to wrap
            client: Reference to the parent AgentInsight client
        ...
        self._otel_span.set_attribute(
            AgentInsightOtelSpanAttributes.OBSERVATION_TYPE, as_type
        )
        self._client = client
        self._observation_type = as_type

        self.trace_id = self._client._get_otel_trace_id(otel_span)
        self.id = self._client._get_otel_span_id(otel_span)

        self._environment = environment or self._client._environment
        if self._environment is not None:
            self._otel_span.set_attribute(
                AgentInsightOtelSpanAttributes.ENVIRONMENT, self._environment
            )

        self._release = release or self._client._release
        if self._release is not None:
            self._otel_span.set_attribute(
                AgentInsightOtelSpanAttributes.RELEASE, self._release
            )
        ...
        if self._otel_span.is_recording():
            ...
            attributes.pop(AgentInsightOtelSpanAttributes.OBSERVATION_TYPE, None)
    ...
    ) -> "AgentInsightObservationWrapper":
    ...
    def set_trace_io(
        ...
        *,
        ...
        .. deprecated::
            ...
            use :meth:`AgentInsight.propagate_attributes` instead.
    ...
    def set_trace_as_public(self) -> "AgentInsightObservationWrapper":
    ...
    def score(
        ...
        *,
        ...
        Example:
            ```python
            with client.start_as_current_observation(name="process-query") as span:
                # Do work
                result = process_data()
    ...
    def score_trace(
        ...
        *,
        ...
        Example:
            ```python
            with client.start_as_current_observation(name="handle-request") as span:
    ...
    def _mask_attribute(self, *, data: Any) -> Any:
        ...
        """
        if not self._client._mask:
            return data

        try:
            return self._client._mask(data=data)
    ...
    def _process_media_in_attribute(
        ...
        *,
        ...
        """
        if self._client._resources is not None:
            return self._client._resources._media_manager._find_and_process_media(
                data=data,
                field=field,
                trace_id=self.trace_id,
                observation_id=self.id,
    ...
    def _set_otel_span_status_if_error(
        ...
        AgentInsight observation level is set to ERROR, ensuring consistency between
        AgentInsight and OpenTelemetry error states.
    ...
    def start_observation(
        ...
        *,
        ...
        if as_type == "event":
            timestamp = time_ns()
            event_span = self._client._otel_tracer.start_span(
        ...
        if not observation_class:
            agentinsight_logger.warning(
                f"Unknown observation type: {as_type}, falling back to AgentInsightSpan"
            )
            observation_class = AgentInsightSpan

        with otel_trace_api.use_span(self._otel_span):
            new_otel_span = self._client._otel_tracer.start_span(name=name)
    ...
    def create_event(
        ...
        *,
        ...
        with otel_trace_api.use_span(self._otel_span):
            new_otel_span = self._client._otel_tracer.start_span(
                name=name, start_time=timestamp
            )
...
class AgentInsightSpan(AgentInsightObservationWrapper):
    """Standard span implementation for general operations in AgentInsight.

    This class represents a general-purpose span that can be used to trace
    any operation in your application. It extends the base AgentInsightObservationWrapper
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new AgentInsightSpan.
...
class AgentInsightGeneration(AgentInsightObservationWrapper):
    """Specialized span implementation for AI model generations in AgentInsight.

    This class represents a generation span specifically designed for tracking
    AI/LLM operations. It extends the base AgentInsightObservationWrapper with specialized
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new AgentInsightGeneration span.
...
class AgentInsightEvent(AgentInsightObservationWrapper):
    """Specialized span implementation for AgentInsight Events."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a new AgentInsightEvent span.
    ...
    def update(
        ...
        **kwargs: Any,
        ...
        """Update is not allowed for AgentInsightEvent because events cannot be updated.

        This method logs a warning and returns self without making changes.

        Returns:
            self: Returns the unchanged AgentInsightEvent instance
        """
        agentinsight_logger.warning(
            "Attempted to update AgentInsightEvent observation. Events cannot be updated after creation."
        )
        return self


class AgentInsightAgent(AgentInsightObservationWrapper):
    """Agent observation for reasoning blocks that act on tools using LLM guidance."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new AgentInsightAgent span."""
        kwargs["as_type"] = "agent"
        super().__init__(**kwargs)

    def set_tool_call(self, name: str, arguments: dict, result: Any) -> None:
        self.update(
            metadata={
...
class AgentInsightTool(AgentInsightObservationWrapper):
    """Tool observation representing external tool calls, e.g., calling a weather API."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new AgentInsightTool span."""
        kwargs["as_type"] = "tool"
        super().__init__(**kwargs)


class AgentInsightChain(AgentInsightObservationWrapper):
    """Chain observation for connecting LLM application steps, e.g. passing context from retriever to LLM."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new AgentInsightChain span."""
        kwargs["as_type"] = "chain"
        super().__init__(**kwargs)


class AgentInsightRetriever(AgentInsightObservationWrapper):
    """Retriever observation for data retrieval steps, e.g. vector store or database queries."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new AgentInsightRetriever span."""
        kwargs["as_type"] = "retriever"
        super().__init__(**kwargs)

    def set_retrieved_documents(self, documents: List[dict]) -> None:
        self.update(metadata={"retrieved_documents": documents})


class AgentInsightEmbedding(AgentInsightObservationWrapper):
    """Embedding observation for LLM embedding calls, typically used before retrieval."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new AgentInsightEmbedding span."""
        kwargs["as_type"] = "embedding"
        super().__init__(**kwargs)


class AgentInsightEvaluator(AgentInsightObservationWrapper):
    """Evaluator observation for assessing relevance, correctness, or helpfulness of LLM outputs."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new AgentInsightEvaluator span."""
        kwargs["as_type"] = "evaluator"
        super().__init__(**kwargs)


class AgentInsightGuardrail(AgentInsightObservationWrapper):
    """Guardrail observation for protection e.g. against jailbreaks or offensive content."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize a new AgentInsightGuardrail span."""
        kwargs["as_type"] = "guardrail"
        super().__init__(**kwargs)

    def set_guard_result(self, passed: bool, reason: str = "") -> None:
        self.update(metadata={"guard_passed": passed, "guard_reason": reason})
...
_OBSERVATION_CLASS_MAP.update(
    {
        ...
        "tool": AgentInsightTool,
        "chain": AgentInsightChain,
        "retriever": AgentInsightRetriever,
        "evaluator": AgentInsightEvaluator,
        "embedding": AgentInsightEmbedding,
        "guardrail": AgentInsightGuardrail,
    }
)
