"""Tracer implementation for AgentInsight OpenTelemetry integration.

This module provides the Tracer class, a thread-safe singleton that manages OpenTelemetry
tracing infrastructure for AgentInsight. It handles tracer initialization, span processors,
API clients, and coordinates background tasks for efficient data processing and media handling.

Key features:
- Thread-safe OpenTelemetry tracer with AgentInsight-specific span processors and sampling
...
from agentinsight._client.constants import AGENTINSIGHT_TRACER_NAME
...
from agentinsight._task_manager.media_manager import MediaManager
from agentinsight._task_manager.media_upload_consumer import MediaUploadConsumer
from agentinsight._task_manager.score_ingestion_consumer import ScoreIngestionConsumer
...
from agentinsight._utils.request import AgentInsightClient
from agentinsight.api import AsyncLangfuseAPI, LangfuseAPI
from agentinsight.logger import agentinsight_logger
from agentinsight.types import MaskFunction

from .._version import __version__ as agentinsight_version


class AgentInsightResourceManager:
    ...
    2. Managing the span processor for exporting spans to the AgentInsight API
    3. Creating and managing AgentInsight API clients (both synchronous and asynchronous)
    ...
    _instances: Dict[str, "AgentInsightResourceManager"] = {}
    ...
    def _initialize_instance(
        ...
        *,
        ...
        if tracing_enabled:
            ...
            agentinsight_processor = AgentInsightSpanProcessor(
            ...
            tracer_provider.add_span_processor(agentinsight_processor)
        ...
        self.api = LangfuseAPI(
            ...
            x_langfuse_sdk_version=agentinsight_version,
        ...
        score_ingestion_client = AgentInsightClient(
            public_key=self.public_key,
            secret_key=secret_key,
            base_url=base_url,
            version=agentinsight_version,
        ...
        self._media_upload_enabled = os.environ.get(
            AGENTINSIGHT_MEDIA_UPLOAD_ENABLED, "True"
        ...
        media_upload_thread_count = media_upload_thread_count or max(
            int(os.getenv(AGENTINSIGHT_MEDIA_UPLOAD_THREAD_COUNT, 1)), 1
        )
        ...
        agentinsight_logger.info(
            f"Startup: AgentInsight tracer successfully initialized | "
    ...
    def flush(self, timeout: Optional[float] = None) -> bool:
        if self.tracer_provider is not None and not isinstance(
            ...
            if timeout is not None:
                self.tracer_provider.force_flush(timeout_millis=int(timeout * 1000))
            else:
                self.tracer_provider.force_flush()
            agentinsight_logger.debug("Successfully flushed OTEL tracer provider")

        self._score_ingestion_queue.join()
        agentinsight_logger.debug("Successfully flushed score ingestion queue")

        self._media_upload_queue.join()
        agentinsight_logger.debug("Successfully flushed media upload queue")
...
def _init_tracer_provider(
    *,
    ...
    environment = environment or os.environ.get(AGENTINSIGHT_TRACING_ENVIRONMENT)
    release = (
        release or os.environ.get(AGENTINSIGHT_RELEASE) or get_common_release_envs()
    )

    resource_attributes = {
        AgentInsightOtelSpanAttributes.ENVIRONMENT: environment,
        AgentInsightOtelSpanAttributes.RELEASE: release,
    }
