"""Attribute propagation utilities for AgentInsight OpenTelemetry integration.
...
def propagate_attributes(
    ...
    **Why this matters**: AgentInsight aggregation queries (e.g., total cost by user_id,
    ...
    Example:
        ...
        with client.start_as_current_observation(name="user_workflow") as span:
            with client.propagate_attributes(
                ...
                with client.start_observation(name="llm_call") as llm_span:
                    # This span inherits: user_id, session_id, experiment, environment
                    ...

                with client.start_generation(name="completion") as gen:
        ...
        ```python
        with client.start_as_current_observation(name="workflow") as span:
            # These spans WON'T have user_id
            early_span = client.start_observation(name="early_work")
            early_span.end()

            # Set attributes in the middle
            with client.propagate_attributes(user_id="user_123"):
                # Only spans created AFTER this point will have user_id
                late_span = client.start_observation(name="late_work")
                late_span.end()
        ...
        with client.start_as_current_observation(name="api_request"):
            with client.propagate_attributes(
                user_id="user_123",
                session_id="session_abc",
...
def _get_propagated_attributes_from_context(
    ...
    for baggage_key, baggage_value in baggage_entries.items():
        if baggage_key.startswith(AGENTINSIGHT_BAGGAGE_PREFIX):
    ...
    if (
        ...
        propagated_attributes[AgentInsightOtelSpanAttributes.ENVIRONMENT] = (
...
def _validate_propagated_value(
    *, value: Any, key: str
    ...
    if not isinstance(value, str):
        agentinsight_logger.warning(  # type: ignore
...
AGENTINSIGHT_BAGGAGE_PREFIX = "langfuse_"


def _get_propagated_baggage_key(key: str) -> str:
    return f"{AGENTINSIGHT_BAGGAGE_PREFIX}{key}"


def _get_span_key_from_baggage_key(key: str) -> Optional[str]:
    if not key.startswith(AGENTINSIGHT_BAGGAGE_PREFIX):
        return None

    suffix = key[len(AGENTINSIGHT_BAGGAGE_PREFIX) :]
...
def _get_propagated_span_key(key: str) -> str:
    return {
        "session_id": AgentInsightOtelSpanAttributes.TRACE_SESSION_ID,
        "user_id": AgentInsightOtelSpanAttributes.TRACE_USER_ID,
        "version": AgentInsightOtelSpanAttributes.VERSION,
        "tags": AgentInsightOtelSpanAttributes.TRACE_TAGS,
        "trace_name": AgentInsightOtelSpanAttributes.TRACE_NAME,
        "metadata": AgentInsightOtelSpanAttributes.TRACE_METADATA,
        "experiment_id": AgentInsightOtelSpanAttributes.EXPERIMENT_ID,
        "experiment_name": AgentInsightOtelSpanAttributes.EXPERIMENT_NAME,
        "experiment_metadata": AgentInsightOtelSpanAttributes.EXPERIMENT_METADATA,
        "experiment_dataset_id": AgentInsightOtelSpanAttributes.EXPERIMENT_DATASET_ID,
        "experiment_item_id": AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_ID,
        "experiment_item_metadata": AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_METADATA,
        "experiment_item_root_observation_id": AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_ROOT_OBSERVATION_ID,
    }.get(key) or f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.{key}"
