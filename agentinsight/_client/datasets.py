...
class DatasetClient:
    """Class for managing datasets in AgentInsight.
    ...
    Example:
        ...
        dataset = agentinsight.get_dataset("<dataset_name>")
    ...
    def __init__(
        ...
        *,
        dataset: Dataset,
        items: List[DatasetItem],
        agentinsight_client: "AgentInsight",
        ...
        self._agentinsight_client: "AgentInsight" = agentinsight_client
    ...
    def run_experiment(
        ...
        *,
        ...
        """Run an experiment on this AgentInsight dataset with automatic tracking.

        This is a convenience method that runs an experiment using all items in this
        dataset. It automatically creates a dataset run in AgentInsight for tracking and
        comparison purposes, linking all experiment results to the dataset.

        Key benefits of using dataset.run_experiment():
        - Automatic dataset run creation and linking in AgentInsight UI
        ...
        - Automatic URL generation for viewing results in AgentInsight dashboard

        Args:
            name: Human-readable name for the experiment run. This will be used as
                the dataset run name in AgentInsight for tracking and identification.
            run_name: Optional exact name for the dataset run. If provided, this will be
                used as the exact dataset run name in AgentInsight. If not provided, this will
            ...
            description: Optional description of the experiment's purpose, methodology,
                or what you're testing. Appears in the AgentInsight UI for context.
        ...
        Returns:
            ...
            - dataset_run_id: ID of the created dataset run in AgentInsight.
            - dataset_run_url: Direct URL to view the experiment results in AgentInsight UI.
        ...
        Raises:
            ValueError: If the dataset has no items or no AgentInsight client is available.

        Examples:
            Basic dataset experiment:
            ```python
            dataset = client.get_dataset("qa-evaluation-set")
            ...
            ```python
            dataset = client.get_dataset("content-generation-benchmark")
            ...
            # Results are automatically linked to dataset in AgentInsight
            print(f"Experiment completed! View in AgentInsight: {result['dataset_run_url']}")
            ...
            dataset = client.get_dataset("model-benchmark")
            ...
            # Both experiments are now visible in AgentInsight for easy comparison
            print("Compare results in AgentInsight:")
            print(f"GPT-4: {result_gpt4.dataset_run_url}")
            print(f"Custom: {result_custom.dataset_run_url}")
            ```

        Note:
            - All experiment results are automatically tracked in AgentInsight as dataset runs
        ...
        """
        return self._agentinsight_client.run_experiment(
            name=name,
            run_name=run_name,
