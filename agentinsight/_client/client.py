"""AgentInsight OpenTelemetry integration module.

This module implements AgentInsight's core observability functionality on top of the OpenTelemetry (OTel) standard.
"""
...
from agentinsight._client.attributes import (
...
from agentinsight._client.environment_variables import (
...
from agentinsight._client.exceptions import AgentInsightAuthenticationError
from agentinsight._client.propagation import (
...
from agentinsight._client.span import (
    AgentInsightAgent,
    AgentInsightChain,
...
from agentinsight._client.utils import get_sha256_hash_hex, run_async_safely
from agentinsight._utils import _get_timestamp
from agentinsight._utils.environment import get_common_release_envs
from agentinsight._utils.parse_error import handle_fern_exception
from agentinsight._utils.prompt_cache import PromptCache
from agentinsight.api import (
...
from agentinsight.experiment import (
    Evaluation,
    EvaluatorFunction,
    ExperimentData,
    ExperimentItem,
...
from agentinsight.model import (
    ChatMessageDict,
...
from agentinsight.types import MaskFunction, ScoreDataType, SpanLevel, TraceContext

if TYPE_CHECKING:
    from agentinsight._client.config import AgentInsightConfig

Serializable = Union[str, int, float, bool, None, List[Any], Dict[str, Any]]


class AgentInsight:
    """Main client for AgentInsight tracing and platform features.

    This class provides an interface for creating and managing traces, spans,
    and generations in AgentInsight as well as interacting with the AgentInsight API.
    ...
    Attributes:
        api: Synchronous API client for AgentInsight backend communication
        async_api: Asynchronous API client for AgentInsight backend communication
        _otel_tracer: Internal Tracer instance managing OpenTelemetry components

    Parameters:
        public_key (Optional[str]): Your AgentInsight public API key. Can also be set via AGENTINSIGHT_PUBLIC_KEY environment variable.
        secret_key (Optional[str]): Your AgentInsight secret API key. Can also be set via AGENTINSIGHT_SECRET_KEY environment variable.
        base_url (Optional[str]): The AgentInsight API base URL. Defaults to "https://agent.goldebridge.com". Can also be set via AGENTINSIGHT_BASE_URL environment variable.
        host (Optional[str]): Deprecated. Use base_url instead. The AgentInsight API host URL. Defaults to "https://agent.goldebridge.com".
        timeout (Optional[int]): Timeout in seconds for API requests. Defaults to 5 seconds.
        httpx_client (Optional[httpx.Client]): Custom httpx client for making non-tracing HTTP requests. If not provided, a default client will be created.
        debug (bool): Enable debug logging. Defaults to False. Can also be set via AGENTINSIGHT_DEBUG environment variable.
        tracing_enabled (Optional[bool]): Enable or disable tracing. Defaults to True. Can also be set via AGENTINSIGHT_TRACING_ENABLED environment variable.
        flush_at (Optional[int]): Number of spans to batch before sending to the API. Defaults to 512. Can also be set via AGENTINSIGHT_FLUSH_AT environment variable.
        flush_interval (Optional[float]): Time in seconds between batch flushes. Defaults to 5 seconds. Can also be set via AGENTINSIGHT_FLUSH_INTERVAL environment variable.
        environment (Optional[str]): Environment name for tracing. Default is 'default'. Can also be set via AGENTINSIGHT_TRACING_ENVIRONMENT environment variable. Can be any lowercase alphanumeric string with hyphens and underscores that does not start with 'agentinsight'.
        release (Optional[str]): Release version/hash of your application. Used for grouping analytics by release.
        media_upload_thread_count (Optional[int]): Number of background threads for handling media uploads. Defaults to 1. Can also be set via AGENTINSIGHT_MEDIA_UPLOAD_THREAD_COUNT environment variable.
        sample_rate (Optional[float]): Sampling rate for traces (0.0 to 1.0). Defaults to 1.0 (100% of traces are sampled). Can also be set via AGENTINSIGHT_SAMPLE_RATE environment variable.
        ...
        blocked_instrumentation_scopes (Optional[List[str]]): Deprecated. Use `should_export_span` instead. Equivalent behavior:
            ```python
            from agentinsight.span_filter import is_default_export_span
            ...
            ```
        should_export_span (Optional[Callable[[ReadableSpan], bool]]): Callback to decide whether to export a span. If omitted, AgentInsight uses the default filter (AgentInsight SDK spans, spans with `gen_ai.*` attributes, and known LLM instrumentation scopes).
        ...
        tracer_provider(Optional[TracerProvider]): OpenTelemetry TracerProvider to use for AgentInsight. This can be useful to set to have disconnected tracing between AgentInsight and other OpenTelemetry-span emitting libraries. Note: To track active spans, the context is still shared between TracerProviders. This may lead to broken trace trees.
        span_exporter (Optional[SpanExporter]): Custom OpenTelemetry span exporter for the AgentInsight span processor. If omitted, AgentInsight creates an OTLPSpanExporter pointed at the AgentInsight OTLP endpoint. If provided, AgentInsight does not wire `base_url`, exporter headers, exporter auth, or exporter timeout into it. Configure endpoint, headers, and timeout on the exporter instance directly. If you are sending spans to AgentInsight v4 or using AgentInsight Cloud Fast Preview, include `x-agentinsight-ingestion-version=4` on the exporter to enable real time processing of exported spans.
    ...
    Example:
        ...
        agentinsight = AgentInsight(
            ...
            host="https://agent.goldebridge.com",  # Optional, default shown
        )

        # Create a trace span
        with agentinsight.start_as_current_observation(name="process-query") as span:
    ...
    _resources: Optional[AgentInsightResourceManager] = None
    ...
    def __init__(
        ...
        *,
        ...
        self._base_url = (
            base_url
            or os.environ.get(AGENTINSIGHT_BASE_URL)
            or host
            or os.environ.get(AGENTINSIGHT_HOST, "https://agent.goldebridge.com")
        )
        if host is not None:
            warnings.warn(
                "'host' parameter is deprecated. Use 'base_url' instead.",
                DeprecationWarning,
                stacklevel=2,
            )
        self._environment = environment or cast(
            str, os.environ.get(AGENTINSIGHT_TRACING_ENVIRONMENT)
        )
        self._release = (
            release
            or os.environ.get(AGENTINSIGHT_RELEASE, None)
        ...
        sample_rate = (
            ...
            else float(os.environ.get(AGENTINSIGHT_SAMPLE_RATE, 1.0))
        )
        ...
        timeout = (
            ...
            else int(os.environ.get(AGENTINSIGHT_TIMEOUT, 5))
        )

        if os.environ.get(AGENTINSIGHT_TRACING_ENABLED, "").lower() == "false":
            self._tracing_enabled = False
        elif tracing_enabled is not None:
            self._tracing_enabled = tracing_enabled
        else:
            self._tracing_enabled = True
        if not self._tracing_enabled:
            agentinsight_logger.info(
                "Configuration: AgentInsight tracing is explicitly disabled. No data will be sent to the AgentInsight API."
            )

        debug = (
            debug
            if debug
            else (os.getenv(AGENTINSIGHT_DEBUG, "false").lower() == "true")
        )
        if debug:
            agentinsight_logger.setLevel(logging.DEBUG)
            if not agentinsight_logger.handlers:
        ...
        if public_key is None:
            if strict_mode:
                msg = (
                    "Authentication error: AgentInsight client initialized without public_key. "
                    "Provide a public_key parameter or set AGENTINSIGHT_PUBLIC_KEY environment variable."
                )
                raise AgentInsightAuthenticationError(msg)
            agentinsight_logger.warning(
                "Authentication error: AgentInsight client initialized without public_key. Client will be disabled. "
                "Provide a public_key parameter or set AGENTINSIGHT_PUBLIC_KEY environment variable. "
            )
            self._tracing_enabled = False
        ...
        if secret_key is None:
            if strict_mode:
                msg = (
                    "Authentication error: AgentInsight client initialized without secret_key. "
                    "Provide a secret_key parameter or set AGENTINSIGHT_SECRET_KEY environment variable."
                )
                raise AgentInsightAuthenticationError(msg)
            agentinsight_logger.warning(
                "Authentication error: AgentInsight client initialized without secret_key. Client will be disabled. "
                "Provide a secret_key parameter or set AGENTINSIGHT_SECRET_KEY environment variable. "
            )
            self._tracing_enabled = False
            self._otel_tracer = otel_trace_api.NoOpTracer()
        ...
        if os.environ.get("OTEL_SDK_DISABLED", "false").lower() == "true":
            agentinsight_logger.warning(
                "OTEL_SDK_DISABLED is set. AgentInsight tracing will be disabled and no traces will appear in the UI."
            )
        ...
        self._resources = AgentInsightResourceManager(
    ...
    @classmethod
    def from_config(cls, config: "AgentInsightConfig") -> "AgentInsight":
        return cls(
            ...
            media_upload_thread_count=config.media_upload_thread_count,
            sample_rate=config.sample_rate,
            mask=config.mask,
            should_export_span=config.should_export_span,
    ...
    def start_observation(
        ...
        *,
        ...
        input: Optional[Serializable] = None,
        output: Optional[Serializable] = None,
        metadata: Optional[Serializable] = None,
            ...
            if trace_id:
                ...
                with otel_trace_api.use_span(
                    ...
                    otel_span.set_attribute(
                        AgentInsightOtelSpanAttributes.AS_ROOT, True
                    )
    ...
    def _create_observation_from_otel_span(
        ...
        *,
        ...
        if as_type in get_observation_types_list(ObservationTypeGenerationLike):
            ...
            # issue is that AgentInsightEvent could be returned and that classes have diff. args
    ...
    ) -> _AgnosticContextManager[AgentInsightGeneration]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[AgentInsightSpan]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[AgentInsightAgent]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[AgentInsightTool]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[AgentInsightChain]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[AgentInsightRetriever]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[AgentInsightEvaluator]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        prompt: Optional[PromptClient] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[AgentInsightEmbedding]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        status_message: Optional[str] = None,
        end_on_exit: Optional[bool] = None,
    ) -> _AgnosticContextManager[AgentInsightGuardrail]: ...
    ...
    def start_as_current_observation(
        ...
        *,
        ...
        Example:
            ```python
            # Create a span
            with AgentInsight.start_as_current_observation(name="process-query", as_type="span") as span:
                # Do work
                result = process_data()
            ...
            with AgentInsight.start_as_current_observation(name="web-search", as_type="tool") as tool:
                # Do tool work
                results = search_web(query)
                tool.update(output=results)

            # Create a generation observation
            with AgentInsight.start_as_current_observation(
    ...
    def trace(self, name: str, **kwargs: Any) -> _AgnosticContextManager:
        return self.start_as_current_observation(name=name, **kwargs)

    def generation(
        self, name: str, model: Optional[str] = None, **kwargs: Any
    ) -> _AgnosticContextManager:
        return self.start_as_current_observation(
            name=name, as_type="generation", model=model, **kwargs
        )

    def span(self, name: str, **kwargs: Any) -> _AgnosticContextManager:
        return self.start_as_current_observation(name=name, as_type="span", **kwargs)

    def _get_span_class(
        self,
        as_type: ObservationTypeLiteral,
    ) -> Union[
        Type[AgentInsightAgent],
        Type[AgentInsightTool],
        Type[AgentInsightChain],
        Type[AgentInsightRetriever],
        Type[AgentInsightEvaluator],
        Type[AgentInsightEmbedding],
        Type[AgentInsightGuardrail],
        Type[AgentInsightGeneration],
        Type[AgentInsightEvent],
        Type[AgentInsightSpan],
    ]:
        """Get the appropriate span class based on as_type."""
        normalized_type = as_type.lower()

        if normalized_type == "agent":
            return AgentInsightAgent
        elif normalized_type == "tool":
            return AgentInsightTool
        elif normalized_type == "chain":
            return AgentInsightChain
        elif normalized_type == "retriever":
            return AgentInsightRetriever
        elif normalized_type == "evaluator":
            return AgentInsightEvaluator
        elif normalized_type == "embedding":
            return AgentInsightEmbedding
        elif normalized_type == "guardrail":
            return AgentInsightGuardrail
        elif normalized_type == "generation":
            return AgentInsightGeneration
        elif normalized_type == "event":
            return AgentInsightEvent
        elif normalized_type == "span":
            return AgentInsightSpan
        else:
            return AgentInsightSpan
        ...
        with otel_trace_api.use_span(parent_span):
            with self._start_as_current_otel_span_with_processed_media(
                ...
                if remote_parent_span is not None:
                    current_span._otel_span.set_attribute(
                        AgentInsightOtelSpanAttributes.AS_ROOT, True
                    )
    ...
    def update_current_generation(
        ...
        *,
        ...
        Example:
            ```python
            with AgentInsight.start_as_current_generation(name="answer-query") as generation:
        ...
        if current_otel_span is not None:
            generation = AgentInsightGeneration(
                otel_span=current_otel_span, client=self
            )
    ...
    def update_current_span(
        ...
        *,
        ...
        Example:
            ```python
            with AgentInsight.start_as_current_observation(name="process-data") as span:
                # Initial processing
                result = process_first_part()

                # Update with intermediate results
                client.update_current_span(metadata={"intermediate_result": result})

                # Continue processing
                final_result = process_second_part(result)

                # Final update
                client.update_current_span(output=final_result)
            ```
        """
        if not self._tracing_enabled:
            agentinsight_logger.debug(
    ...
    def set_current_trace_io(
        ...
        *,
        ...
        .. deprecated::
            This is a legacy method for backward compatibility with AgentInsight platform
        ...
        if current_otel_span is not None and current_otel_span.is_recording():
            existing_observation_type = current_otel_span.attributes.get(  # type: ignore[attr-defined]
                AgentInsightOtelSpanAttributes.OBSERVATION_TYPE, "span"
            )
    ...
    def set_current_trace_as_public(self) -> None:
        ...
        without needing to be logged in to AgentInsight. This action cannot be undone
    ...
    def create_event(
        ...
        *,
        ...
        """Create a new AgentInsight observation of type 'EVENT'.

        The created AgentInsight Event observation will be the child of the current span in the context.
        ...
        Returns:
            The AgentInsight Event object

        Example:
            ```python
            event = client.create_event(name="process-event")
            ```
        """
        timestamp = time_ns()
    ...
    def _create_remote_parent_span(
        ...
        if not self._is_valid_trace_id(trace_id):
            agentinsight_logger.warning(
                f"Passed trace ID '{trace_id}' is not a valid 32 lowercase hex char AgentInsight trace id. Ignoring trace ID."
            )

        if parent_span_id and not self._is_valid_span_id(parent_span_id):
            agentinsight_logger.warning(
                f"Passed span ID '{parent_span_id}' is not a valid 16 lowercase hex char AgentInsight span id. Ignoring parent span ID."
            )
    ...
    def _create_observation_id(self, *, seed: Optional[str] = None) -> str:
        """Create a unique observation ID for use with AgentInsight.

        This method generates a unique observation ID (span ID in OpenTelemetry terms)
        for use with various AgentInsight APIs. It can either generate a random ID or
        ...
        correlate an external ID with a AgentInsight observation ID, use the external ID as
        ...
        Example:
            ...
            obs_id = client.create_observation_id()

            # Generate a deterministic ID based on a seed
            user_obs_id = client.create_observation_id(seed="user-123-feedback")

            # Correlate an external item ID with a AgentInsight observation ID
            item_id = "item-789012"
            correlated_obs_id = client.create_observation_id(seed=item_id)

            # Use the ID with AgentInsight APIs
            client.create_score(
                name="relevance",
                value=0.95,
                trace_id=trace_id,
    ...
    def create_trace_id(*, seed: Optional[str] = None) -> str:
        """Create a unique trace ID for use with AgentInsight.

        This method generates a unique trace ID for use with various AgentInsight APIs.
        ...
        This method ensures the generated ID meets this requirement. If you need to
        correlate an external ID with an AgentInsight trace ID, use the external ID as the
        seed to get a valid, deterministic AgentInsight trace id.
        ...
        Returns:
            A 32-character lowercase hexadecimal string representing the AgentInsight trace id.
        ...
        Example:
            ...
            # Generate a deterministic ID based on a seed
            session_trace_id = client.create_trace_id(seed="session-456")

            # Correlate an external ID with an AgentInsight trace ID
            external_id = "external-system-123456"
            correlated_trace_id = client.create_trace_id(seed=external_id)
        ...
        if not seed:
            ...
            return AgentInsight._format_otel_trace_id(trace_id_int)
    ...
    def create_score(
        ...
        *,
        ...
        This method creates a score for evaluating a AgentInsight trace or observation. Scores can be
        ...
        Args:
            ...
            session_id: ID of the AgentInsight session to associate the score with
            dataset_run_id: ID of the AgentInsight dataset run to associate the score with
            trace_id: ID of the AgentInsight trace to associate the score with
            ...
            config_id: Optional ID of a score config defined in AgentInsight
    ...
    def score_current_trace(
        ...
        *,
        ...
        Example:
            ```python
            with AgentInsight.start_as_current_observation(name="process-user-request") as span:
                # Process request
    ...
    def flush(self, timeout: Optional[float] = None) -> bool:
        """Force flush all pending spans and events to the AgentInsight API.

        This method manually flushes any pending spans, scores, and other events to the
        AgentInsight API. It's useful in scenarios where you want to ensure all data is sent
        before proceeding, without waiting for the automatic flush interval.

        Args:
            timeout: Optional timeout in seconds for the flush operation. If not provided,
                the flush will wait indefinitely.
        ...
        Example:
            ...
            with AgentInsight.start_as_current_observation(name="operation") as span:
                # Do work...
                pass

            # Ensure all data is sent to AgentInsight before proceeding
            client.flush()
        ...
        """
        if self._resources is not None:
            return self._resources.flush(timeout=timeout)
        return True

    def __enter__(self) -> "AgentInsight":
        return self
    ...
    def get_status(self) -> Dict[str, Any]:
        return {
            "is_initialized": self._resources is not None,
            "is_tracing_enabled": self._tracing_enabled,
            "pending_spans": self._resources.pending_spans if self._resources else 0,
            "base_url": self._base_url,
            "environment": self._environment,
            "release": self._release,
        }
    ...
    def shutdown(self) -> None:
        """Shut down the AgentInsight client and flush all pending data.

        This method cleanly shuts down the AgentInsight client, ensuring all pending data
        ...
        Example:
            ```python
            # Initialize AgentInsight
            agentinsight = AgentInsight(public_key="...", secret_key="...")

            # Use AgentInsight throughout your application
            # ...
    ...
    def get_current_trace_id(self) -> Optional[str]:
        ...
        Example:
            ```python
            with AgentInsight.start_as_current_observation(name="process-request") as span:
    ...
    def get_current_observation_id(self) -> Optional[str]:
        ...
        Example:
            ```python
            with AgentInsight.start_as_current_observation(name="process-user-query") as span:
    ...
    def get_trace_url(self, *, trace_id: Optional[str] = None) -> Optional[str]:
        """Get the URL to view a trace in the AgentInsight UI.

        This method generates a URL that links directly to a trace in the AgentInsight UI.
        ...
        Returns:
            A URL string pointing to the trace in the AgentInsight UI,
        ...
        Example:
            ...
            specific_trace_url = client.get_trace_url(trace_id="1234567890abcdef1234567890abcdef")
    ...
    def get_dataset(
        ...
        *,
        ...
        """
        try:
            agentinsight_logger.debug(f"Getting datasets {name}")
    ...
    def run_experiment(
        ...
        *,
        ...
        automatically traces all executions with AgentInsight for observability, runs
        ...
        - Integration with AgentInsight datasets for experiment tracking
        ...
        Args:
            name: Human-readable name for the experiment. Used for identification
                in the AgentInsight UI.
            run_name: Optional exact name for the experiment run. If provided, this will be
                used as the exact dataset run name if the `data` contains AgentInsight dataset items.
            ...
            data: Array of data items to process. Can be either:
                ...
                - List of AgentInsight DatasetItem objects from dataset.items
            ...
            metadata: Optional metadata dictionary to attach to all experiment traces.
                ...
                If `data` are AgentInsight dataset items, the metadata will be attached to the dataset run, too.

        Returns:
            ExperimentResult containing:
            - run_name: The experiment run name. This is equal to the dataset run name if experiment was on AgentInsight dataset.
            ...
            - dataset_run_id: ID of the dataset run (if using AgentInsight datasets)
            - dataset_run_url: Direct URL to view results in AgentInsight UI (if applicable)
        ...
        Examples:
            ...
            Using with AgentInsight datasets:
            ```python
            # Get dataset from AgentInsight
            ...
            # Results automatically linked to dataset in AgentInsight UI
        ...
        Note:
            ...
            - All executions are automatically traced and visible in AgentInsight UI
            - When using AgentInsight datasets, results are automatically linked for easy comparison
        ...
        *,
        ...
        for i, result in enumerate(item_results):
            if isinstance(result, Exception):
                agentinsight_logger.error(f"Item {i} failed: {result}")
        ...
        for run_evaluator in run_evaluators:
            try:
                ...
                agentinsight_logger.error(f"Run evaluator failed: {e}")
        ...
        for evaluation in run_evaluations:
            try:
                ...
                agentinsight_logger.error(f"Failed to store run evaluation: {e}")
        ...
        with self.start_as_current_observation(name=span_name) as span:
                ...
                span._otel_span.set_attributes(
                    {
                        k: v
                        for k, v in {
                            AgentInsightOtelSpanAttributes.ENVIRONMENT: AGENTINSIGHT_SDK_EXPERIMENT_ENVIRONMENT,
                            AgentInsightOtelSpanAttributes.EXPERIMENT_DESCRIPTION: experiment_description,
                            AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_EXPECTED_OUTPUT: _serialize(
                                expected_output
                            ),
                        }.items()
                        if v is not None
            ...
            for evaluator in evaluators:
                try:
                    ...
                    agentinsight_logger.error(f"Evaluator failed: {e}")
            ...
            if composite_evaluator and evaluations:
                try:
                    ...
                    agentinsight_logger.error(f"Composite evaluator failed: {e}")
    ...
    def run_batched_evaluation(
        ...
        *,
        ...
        This method provides a powerful way to evaluate existing data in AgentInsight at scale.
        ...
        Args:
            ...
            filter: Optional JSON filter string for querying items (same format as AgentInsight API). Examples:
                - '{"tags": ["production"]}'
        ...
        Examples:
            ...
            ```python
            from agentinsight import agentinsight, EvaluatorInputs, Evaluation
        ...
        Note:
            ...
            - All Scores are automatically flushed to AgentInsight at the end
    ...
    def create_dataset(
        ...
        *,
        ...
        """Create a dataset with the given name on AgentInsight.
        ...
        Returns:
            Dataset: The created dataset as returned by the AgentInsight API.
        """
        try:
            agentinsight_logger.debug(f"Creating datasets {name}")
    ...
    def create_dataset_item(
        ...
        *,
        ...
        Returns:
            DatasetItem: The created dataset item as returned by the AgentInsight API.
        ...
        Example:
            ...
            # Uploading items to the AgentInsight dataset named "capital_cities"
        ...
        try:
            agentinsight_logger.debug(
                f"Creating dataset item for dataset {dataset_name}"
            )
    ...
    def resolve_media_references(
        ...
        *,
        ...
        the provided AgentInsight client and replaces the reference string with a base64 data URI.
        ...
        Example:
            ...
            result = await AgentInsightMedia.resolve_media_references(obj, agentinsight_client)
            ...
            # }
        """
        return AgentInsightMedia.resolve_media_references(
            agentinsight_client=self,
            obj=obj,
    ...
    def get_prompt(
        ...
        *,
        ...
        Keyword Args:
            ...
            fallback: Union[Optional[List[ChatMessageDict]], Optional[str]]: The prompt string to return if fetching the prompt fails. Important on the first call where no cached prompt is available. Follows AgentInsight prompt formatting with double curly braces for variables. Defaults to None.
        ...
        agentinsight_logger.debug(f"Getting prompt '{cache_key}'")
        ...
        if cached_prompt.is_expired():
            agentinsight_logger.debug(f"Stale prompt '{cache_key}' found in cache.")
            try:
    ...
    def _fetch_prompt_and_update_cache(
        ...
        *,
        ...
        cache_key = PromptCache.generate_cache_key(name, version=version, label=label)
        agentinsight_logger.debug(f"Fetching prompt '{cache_key}' from server...")
    ...
    def create_prompt(
        ...
        *,
        ...
        """Create a new prompt in AgentInsight.
        ...
        """
        try:
            agentinsight_logger.debug(f"Creating prompt {name=}, {labels=}")
    ...
    def update_prompt(
        ...
        *,
        ...
        """Update an existing prompt version in AgentInsight. The AgentInsight SDK prompt cache is invalidated for all prompts witht he specified name.

        Args:
            name (str): The name of the prompt to update.
            version (int): The version number of the prompt to update.
            new_labels (List[str], optional): New labels to assign to the prompt version. Labels are unique across versions. The "latest" label is reserved and managed by AgentInsight. Defaults to [].

        Returns:
            Prompt: The updated prompt from the AgentInsight API.
...
AgentInsightClient = AgentInsight
