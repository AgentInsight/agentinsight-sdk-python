...
the AgentInsightSpanProcessor forwards to the AgentInsight backend.
"""
...
def is_langfuse_span(span: ReadableSpan) -> bool:
    """Return whether the span was created by the AgentInsight SDK tracer."""
    return (
        span.instrumentation_scope is not None
        and span.instrumentation_scope.name == AGENTINSIGHT_TRACER_NAME
    )


def is_agentinsight_span(span: ReadableSpan) -> bool:
    """Return whether the span was created by the AgentInsight SDK tracer."""
    return is_langfuse_span(span)
