"""Span processor for AgentInsight OpenTelemetry integration.

This module defines the AgentInsightSpanProcessor class, which extends OpenTelemetry's
BatchSpanProcessor with AgentInsight-specific functionality. It handles exporting
spans to the AgentInsight API with proper authentication and filtering.

Key features:
- HTTP-based span export to AgentInsight API
- Basic authentication with AgentInsight API keys
...
from agentinsight._client.environment_variables import (
    ...
    AGENTINSIGHT_OTEL_TRACES_EXPORT_PATH,
)
from agentinsight._client.propagation import _get_propagated_attributes_from_context
from agentinsight._client.span_filter import is_default_export_span, is_langfuse_span
from agentinsight._client.utils import span_formatter
from agentinsight._version import __version__ as agentinsight_version
from agentinsight.logger import agentinsight_logger


class AgentInsightSpanProcessor(BatchSpanProcessor):
    """OpenTelemetry span processor that exports spans to the AgentInsight API.

    This processor extends OpenTelemetry's BatchSpanProcessor with AgentInsight-specific functionality:
    ...
    4. HTTP-based span export to the AgentInsight OTLP endpoint
    5. Debug logging for span processing operations
    6. Authentication with AgentInsight API using Basic Auth
    ...
    It integrates with OpenTelemetry's standard span lifecycle, adding AgentInsight-specific
    ...
    def __init__(
        ...
        *,
        ...
        env_flush_at = os.environ.get(AGENTINSIGHT_FLUSH_AT, None)
        flush_at = flush_at or int(env_flush_at) if env_flush_at is not None else None

        env_flush_interval = os.environ.get(AGENTINSIGHT_FLUSH_INTERVAL, None)
        flush_interval = (
        ...
        if span_exporter is None:
            ...
            traces_export_path = os.environ.get(
                AGENTINSIGHT_OTEL_TRACES_EXPORT_PATH, None
            )
    ...
    def on_end(self, span: ReadableSpan) -> None:
        ...
        if is_langfuse_span(span) and not self._is_agentinsight_project_span(span):
            agentinsight_logger.debug(
    ...
    def _is_agentinsight_project_span(self, span: ReadableSpan) -> bool:
