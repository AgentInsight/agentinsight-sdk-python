"""If you use the OpenAI Python SDK, you can use the AgentInsight drop-in replacement to get full logging by changing only the import.
...
AgentInsight automatically tracks:
...
class OpenAiArgsExtractor:
    def __init__(
        ...
        agentinsight_prompt: Optional[Any] = None,
        agentinsight_public_key: Optional[str] = None,
        ...
        **kwargs: Any,
        ...
        self.args["name"] = name
        self.args["agentinsight_public_key"] = (
            langfuse_public_key or agentinsight_public_key
        )
        self.args["agentinsight_prompt"] = langfuse_prompt or agentinsight_prompt
        self.args["trace_id"] = trace_id
        self.args["parent_observation_id"] = parent_observation_id

        self.kwargs = kwargs

    def get_agentinsight_args(self) -> Any:
...
def _agentinsight_wrapper(func: Any) -> Any:
    def _with_agentinsight(open_ai_definitions: Any) -> Any:
        def wrapper(wrapped: Any, instance: Any, args: Any, kwargs: Any) -> Any:
            return func(open_ai_definitions, wrapped, args, kwargs)

        return wrapper

    return _with_agentinsight
...
def _extract_chat_prompt(kwargs: Any) -> Any:
    ...
    if prompt:
        # uf user provided functions, we need to send these together with messages to AgentInsight
        prompt.update(
            {
                "messages": [
        ...
        # vanilla case, only send messages in openai format to AgentInsight
                ...
                processed_content.append(
                    {
                        "type": "input_audio",
                        "input_audio": {
                            "data": AgentInsightMedia(base64_data_uri=base64_data_uri),
                            "format": format,
                        },
                    }
                )
...
def _extract_chat_response(kwargs: Any) -> Any:
    ...
    if kwargs.get("audio") is not None:
        ...
        if "data" in audio and audio["data"] is not None:
            ...
            audio["data"] = AgentInsightMedia(base64_data_uri=base64_data_uri)
...
def _get_agentinsight_data_from_kwargs(resource: OpenAiDefinition, kwargs: Any) -> Any:
    default_name = (
    ...
    agentinsight_public_key = kwargs.get("agentinsight_public_key", None) or kwargs.get(
        "langfuse_public_key", None
    )
    if agentinsight_public_key is not None and not isinstance(
        agentinsight_public_key, str
    ):
        raise TypeError("agentinsight_public_key must be a string")
    ...
    agentinsight_prompt = kwargs.get("agentinsight_prompt", None) or kwargs.get(
    ...
    return {
        ...
        "model": model or None,
        "prompt": agentinsight_prompt,
    }


def _create_agentinsight_update(
    completion: Any,
...
def _get_agentinsight_data_from_default_response(
    ...
    if not _openai_stream_iter_hook_installed:
        ...
        def traced_iter(self: Any) -> Any:
            try:
                ...
                finalize_once = getattr(self, "_agentinsight_finalize_once", None)
...
def _instrument_openai_stream(
    *,
    ...
    if not hasattr(response, "_iterator"):
        return AgentInsightResponseGeneratorSync(
            resource=resource,
            response=response,
    ...
    response._agentinsight_finalize_once = finalize_once  # type: ignore[attr-defined]
...
def _instrument_openai_async_stream(
    *,
    ...
    if not hasattr(response, "_iterator"):
        return AgentInsightResponseGeneratorAsync(
            resource=resource,
            response=response,
            generation=generation,
        )
...
@_agentinsight_wrapper
def _wrap(
    open_ai_resource: OpenAiDefinition, wrapped: Any, args: Any, kwargs: Any
) -> Any:
    arg_extractor = OpenAiArgsExtractor(*args, **kwargs)
    agentinsight_args = arg_extractor.get_agentinsight_args()

    agentinsight_data = _get_agentinsight_data_from_kwargs(
        open_ai_resource, agentinsight_args
    )
    agentinsight_client = get_client(
        public_key=agentinsight_args["agentinsight_public_key"]
    )
    ...
    generation = agentinsight_client.start_observation(
        ...
        name=agentinsight_data["name"],
        input=agentinsight_data.get("input", None),
        metadata=agentinsight_data.get("metadata", None),
        model_parameters=agentinsight_data.get("model_parameters", None),
        trace_context={
            "trace_id": cast(str, agentinsight_data.get("trace_id", None)),
            "parent_span_id": cast(
                str, agentinsight_data.get("parent_observation_id", None)
            ),
        },
        model=agentinsight_data.get("model", None),
        prompt=agentinsight_data.get("prompt", None),
    )
    ...
    try:
        ...
        if _is_openai_v1() and isinstance(openai_response, openai.Stream):
            ...
            model, completion, usage = _get_agentinsight_data_from_default_response(
                open_ai_resource,
...
class AgentInsightResponseGeneratorSync:
    def __init__(
        self,
        *,
        resource: Any,
...
class AgentInsightResponseGeneratorAsync:
    def __init__(
        self,
        *,
        resource: Any,
