"""AgentInsight experiment functionality for running and evaluating tasks on datasets.

This module provides the core experiment functionality for the AgentInsight Python SDK,
...
from agentinsight.api import DatasetItem
from agentinsight.logger import agentinsight_logger as logger
from agentinsight.types import ExperimentScoreType

if TYPE_CHECKING:
    from agentinsight._client.client import AgentInsight
    from agentinsight.batch_evaluation import CompositeEvaluatorFunction


class LocalExperimentItem(TypedDict, total=False):
    """Structure for local experiment data items (not from AgentInsight datasets).

    This TypedDict defines the structure for experiment items when using local data
    rather than AgentInsight-hosted datasets. All fields are optional to provide
...
- DatasetItem: Items from AgentInsight datasets with .input, .expected_output, .metadata attributes
"""
...
- List[DatasetItem]: Items from a AgentInsight dataset (typically from dataset.items)
"""
...
class Evaluation:
    ...
    Attributes:
        ...
        comment: Optional human-readable explanation of the evaluation result.
            ...
            special conditions. Displayed in AgentInsight UI for interpretability.
        ...
        config_id: Optional AgentInsight score config ID.

    Examples:
        Basic accuracy evaluation:
        ```python
        from agentinsight import Evaluation
...
class ExperimentItemResult:
    ...
    Attributes:
        item: The original experiment item that was processed. Can be either
            ...
            or a DatasetItem from AgentInsight datasets.
        ...
        trace_id: Optional AgentInsight trace ID for this item's execution. Used
            to link the experiment result with the detailed trace in AgentInsight UI.
        dataset_run_id: Optional dataset run ID if this item was part of a
            AgentInsight dataset. None for local experiments.
    ...
    Examples:
        ...
        # AgentInsight dataset item (object with attributes)
        else:
            input_data = item_result.item.input
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            trace_id: Optional AgentInsight trace ID for this item's execution.
            dataset_run_id: Optional dataset run ID if this item was part of a AgentInsight dataset.
...
class ExperimentResult:
    ...
    Attributes:
        ...
        experiment_id: ID of the experiment run propagated across all items. For
            AgentInsight datasets, this matches the dataset run ID. For local experiments,
            this is a stable SDK-generated identifier for the run.
        dataset_run_id: Optional ID of the dataset run in AgentInsight (when using AgentInsight datasets).
        dataset_run_url: Optional direct URL to view the experiment results in AgentInsight UI.
    ...
    Examples:
        ...
        Usage with AgentInsight datasets:
        ```python
        dataset = client.get_dataset("qa-eval-set")
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            dataset_run_id: Optional ID of the dataset run (for AgentInsight datasets).
            dataset_run_url: Optional URL to view results in AgentInsight UI.
        """
        self.name = name
    ...
    def format(self, *, include_item_results: bool = False) -> str:
        ...
        - Links to view detailed results in AgentInsight UI (when available)
        ...
        Returns:
            ...
            - Dataset run URL for viewing in AgentInsight UI (if applicable)
        ...
        **kwargs: Dict[str, Any],
        ...
        Args:
            item: The experiment item to process. Can be either:
                ...
                - AgentInsight DatasetItem object with .input, .expected_output attributes
        ...
        Returns:
            ...
            - Traced automatically in AgentInsight for observability
        ...
        **kwargs: Dict[str, Any],
        ...
        Args:
            item_results: List of results from all successfully processed experiment items.
                ...
                - trace_id: AgentInsight trace ID for this execution
                - dataset_run_id: Dataset run ID (if using AgentInsight datasets)
...
def create_evaluator_from_autoevals(
    ...
    """Create an AgentInsight evaluator from an autoevals evaluator.
    ...
    Returns:
        An AgentInsight-compatible evaluator function
    """

    def agentinsight_evaluator(
        *,
        input: Any,
        output: Any,
        expected_output: Any,
        metadata: Optional[Dict[str, Any]],
        **agentinsight_kwargs: Dict[str, Any],
    ) -> Evaluation:
...
class RunnerContext:
    ...
    def __init__(
        ...
        *,
        ...
        Typically called by the ``agentinsight/experiment-action`` GitHub Action,
        ...
        Args:
            client: Initialized AgentInsight SDK client used to execute the
                experiment. The action creates this from the
                ``agentinsight_public_key`` / ``agentinsight_secret_key`` /
                ``agentinsight_base_url`` inputs.
