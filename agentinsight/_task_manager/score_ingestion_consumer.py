...
from agentinsight._utils.parse_error import handle_exception
from agentinsight._utils.request import AgentInsightClient, APIError
...
MAX_EVENT_SIZE_BYTES = int(
    os.environ.get("AGENTINSIGHT_MAX_EVENT_SIZE_BYTES", 1_000_000)
)
MAX_BATCH_SIZE_BYTES = int(
    os.environ.get("AGENTINSIGHT_MAX_BATCH_SIZE_BYTES", 2_500_000)
)
...
class ScoreIngestionConsumer(threading.Thread):
    ...
    def _upload_batch(self, batch: List[Any]) -> None:
        logger.debug(
            f"API: Uploading batch of {len(batch)} score events to AgentInsight API"
        )

        metadata = ScoreIngestionMetadata(
            batch_size=len(batch),
            sdk_name="python",
            sdk_version=agentinsight_version,
        ...
        logger.debug(
            f"API: Successfully sent {len(batch)} score events to AgentInsight API in batch mode"
        )
