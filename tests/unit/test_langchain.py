...
callback_handler_module = importlib.import_module(
    "agentinsight.langchain.CallbackHandler"
)
...
def test_chat_model_callback_exports_generation_span(
    ...
    assert (
        generation_span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_TYPE]
        == "generation"
    )
    assert json_attr(
        generation_span, AgentInsightOtelSpanAttributes.OBSERVATION_INPUT
    ) == [{"role": "user", "content": "hello"}]
    assert json_attr(
        generation_span, AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT
    ) == {
        "role": "assistant",
        "content": "bonjour",
    }
    assert (
        generation_span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_MODEL]
        == "gpt-4o-mini"
    )
    assert json_attr(
        generation_span, AgentInsightOtelSpanAttributes.OBSERVATION_USAGE_DETAILS
    ) == {
        "prompt_tokens": 4,
        "completion_tokens": 2,
        "total_tokens": 6,
...
def test_llm_callback_exports_generation_span(langfuse_memory_client, get_span):
    ...
    assert (
        span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT]
        == "sockzilla"
    )
    assert (
        span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_MODEL]
...
def test_chat_model_error_marks_generation_error(langfuse_memory_client, get_span):
    ...
    assert span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_LEVEL] == "ERROR"
    assert (
        "boom"
        in span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_STATUS_MESSAGE]
    )
...
def test_root_chain_metadata_propagates_trace_name(
    ...
    assert (
        root_span.attributes[AgentInsightOtelSpanAttributes.TRACE_NAME]
        == "langchain-trace-name"
    )
    assert (
        generation_span.attributes[AgentInsightOtelSpanAttributes.TRACE_NAME]
        == "langchain-trace-name"
    )
    assert (
        f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.langfuse_trace_name"
...
def test_root_chain_exports_when_end_runs_in_copied_context(
    ...
    assert root_span.attributes[AgentInsightOtelSpanAttributes.TRACE_NAME] == (
        "async-root-trace"
    )
...
def test_control_flow_errors_use_default_level_and_keep_status_message(
    ...
    handler._agentinsight_client.flush()
    ...
    for span_name, message in [
        ...
        assert (
            span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_LEVEL]
            == "DEFAULT"
        )
        assert (
            span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_STATUS_MESSAGE]
            == message
        )
