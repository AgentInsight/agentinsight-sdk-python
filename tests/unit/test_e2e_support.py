...
pytest.importorskip(
    "tests.support",
    reason="E2E support module not available in this distribution",
)

from types import SimpleNamespace

from agentinsight.api.commons.errors.not_found_error import NotFoundError
from tests.support.api_wrapper import AgentInsightAPI as SupportAgentInsightAPI
    ...
    class FakeTraceService:
        def get(self, trace_id):
            ...
            if attempts["count"] < 3:
                raise NotFoundError(
                    body={
                        "error": "AgentInsightNotFoundError",
...
def test_raw_api_wrapper_retries_not_found_payload(monkeypatch):
    ...
    api = SupportAgentInsightAPI(
        username="user", password="pass", base_url="http://test"
    )
