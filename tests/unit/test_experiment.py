"""Tests for ``AgentInsight.experiment`` — ``RunnerContext`` and ``RegressionError``."""
...
from agentinsight import AgentInsight, RegressionError, RunnerContext
...
def _make_ctx(**kwargs) -> RunnerContext:
    client = MagicMock(spec=AgentInsight)
...
class TestSignatureDriftGuard:
    """Fails loudly if ``AgentInsight.run_experiment`` grows a parameter that is
    ...
    # ``AgentInsight.experiment`` to break the circular dependency with
    # ``AgentInsight.batch_evaluation``, so its forward-ref must be resolved
    # explicitly when inspecting annotations.
    LOCALNS = {"CompositeEvaluatorFunction": CompositeEvaluatorFunction}

    def test_no_divergence(self):
        client_param_names = self._param_names(AgentInsight.run_experiment)
        ...
        assert client_param_names == ctx_param_names, (
            ...
            "AgentInsight.run_experiment. Missing: "
            f"{client_param_names - ctx_param_names}. "
            f"Extra: {ctx_param_names - client_param_names}."
        )

        client_hints = get_type_hints(AgentInsight.run_experiment)
