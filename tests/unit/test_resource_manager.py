"""Test the AgentInsightResourceManager and get_client() function."""
...
def test_get_client_preserves_all_settings(monkeypatch):
    ...
    monkeypatch.setenv("AGENTINSIGHT_PUBLIC_KEY", "pk-comprehensive-default")
    monkeypatch.setenv("AGENTINSIGHT_SECRET_KEY", "sk-comprehensive-default")
    monkeypatch.setenv("AGENTINSIGHT_BASE_URL", "http://localhost:3000")
    ...
    original_client = AgentInsight(**settings)
...
def test_get_client_multiple_clients_preserve_different_settings():
    ...
    client_a = AgentInsight(**settings_a)
    client_b = AgentInsight(**settings_b)
...
def test_stop_and_join_consumer_threads_broadcasts_media_shutdown_after_pausing_all():
    ...
    AgentInsightResourceManager._stop_and_join_consumer_threads(fake_resource_manager)
