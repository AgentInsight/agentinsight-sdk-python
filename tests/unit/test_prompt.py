...
from agentinsight._utils.prompt_cache import (
    DEFAULT_PROMPT_CACHE_TTL_SECONDS,
    PromptCache,
    PromptCacheItem,
    PromptCacheTaskManager,
)
from agentinsight.api import NotFoundError, Prompt_Chat, Prompt_Text
from agentinsight.model import ChatPromptClient, TextPromptClient
...
@patch.object(PromptCacheItem, "get_epoch_seconds")
def test_disable_caching_when_ttl_zero(mock_time, langfuse):
    mock_time.return_value = 0
...
@patch.object(PromptCacheItem, "get_epoch_seconds")
def test_get_stale_prompt_when_expired_cache_default_ttl(mock_time, langfuse):
    import logging
...
@patch.object(PromptCacheItem, "get_epoch_seconds")
def test_skip_redundant_refresh_when_cache_already_updated(mock_time, langfuse) -> None:
