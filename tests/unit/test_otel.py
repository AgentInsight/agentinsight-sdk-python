...
from agentinsight._client.attributes import AgentInsightOtelSpanAttributes
...
class TestOTelBase:
    ...
    def tracer_provider(self, memory_exporter):
        ...
        resource = Resource.create({"service.name": "agentinsight-test"})
    ...
    def mock_processor_init(self, monkeypatch, memory_exporter):
        """Mock the AgentInsightSpanProcessor initialization to avoid HTTP traffic."""

        def mock_init(self, **kwargs):
            from opentelemetry.sdk.trace.export import BatchSpanProcessor

            from agentinsight._client.span_filter import is_default_export_span
        ...
        monkeypatch.setattr(
            "agentinsight._client.span_processor.AgentInsightSpanProcessor.__init__",
            mock_init,
        )
    ...
    def agentinsight_client(self, monkeypatch, tracer_provider, mock_processor_init):
        ...
        client._otel_tracer = tracer_provider.get_tracer("agentinsight-test")

        yield client

    @pytest.fixture
    def configurable_agentinsight_client(
...
class TestBasicSpans(TestOTelBase):
    ...
    def test_basic_span_creation(self, agentinsight_client, memory_exporter):
        ...
        self.verify_span_attribute(
            span_data, AgentInsightOtelSpanAttributes.OBSERVATION_TYPE, "span"
        )
    ...
    def test_span_hierarchy(self, agentinsight_client, memory_exporter):
    ...
    def test_update_current_span_name(self, agentinsight_client, memory_exporter):
        """Test updating current span name via update_current_span method."""
        # Create a span using context manager
        with agentinsight_client.start_as_current_observation(
            name="original-current-span"
        ):
            # Update the current span name
            agentinsight_client.update_current_span(name="updated-current-span")
    ...
    def test_span_attributes(self, agentinsight_client, memory_exporter):
        ...
        attributes = span_data["attributes"]
        assert AgentInsightOtelSpanAttributes.OBSERVATION_INPUT in attributes
        assert AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT in attributes
        assert (
            f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.session"
            in attributes
        )

        # Parse JSON attributes
        input_data = json.loads(
            attributes[AgentInsightOtelSpanAttributes.OBSERVATION_INPUT]
        )
        output_data = json.loads(
            attributes[AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT]
        )
        ...
        assert attributes[AgentInsightOtelSpanAttributes.OBSERVATION_LEVEL] == "INFO"
        assert (
            attributes[AgentInsightOtelSpanAttributes.OBSERVATION_STATUS_MESSAGE]
            == "Test status"
        )

    def test_span_name_update(self, agentinsight_client, memory_exporter):
        """Test updating span name via update method."""
        # Create a span with initial name
        span = agentinsight_client.start_observation(name="original-span-name")
    ...
    def test_generation_span(self, agentinsight_client, memory_exporter):
        ...
        assert (
            attributes[AgentInsightOtelSpanAttributes.OBSERVATION_TYPE] == "generation"
        )
        assert attributes[AgentInsightOtelSpanAttributes.OBSERVATION_MODEL] == "gpt-4"

        # Parse complex attributes
        model_params = json.loads(
            attributes[AgentInsightOtelSpanAttributes.OBSERVATION_MODEL_PARAMETERS]
        )
        assert model_params == {"temperature": 0.7, "max_tokens": 100}

        usage = json.loads(
            attributes[AgentInsightOtelSpanAttributes.OBSERVATION_USAGE_DETAILS]
        )
        assert usage == {"input": 10, "output": 5, "total": 15}

    def test_generation_name_update(self, agentinsight_client, memory_exporter):
    ...
    def test_trace_update(self, agentinsight_client, memory_exporter):
        ...
        attributes = span_data["attributes"]
        assert (
            attributes[AgentInsightOtelSpanAttributes.TRACE_NAME]
            == "updated-trace-name"
        )
        assert attributes[AgentInsightOtelSpanAttributes.TRACE_USER_ID] == "test-user"
        assert (
            attributes[AgentInsightOtelSpanAttributes.TRACE_SESSION_ID]
            == "test-session"
        )

        # Handle different serialization formats
        if isinstance(attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS], str):
            tags = json.loads(attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS])
        else:
            tags = list(attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS])

        input_data = json.loads(attributes[AgentInsightOtelSpanAttributes.TRACE_INPUT])
        metadata = attributes[
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.trace-meta"
        ]
    ...
    def test_complex_scenario(self, agentinsight_client, memory_exporter):
        ...
        assert (
            main["attributes"][AgentInsightOtelSpanAttributes.TRACE_NAME]
            == "complex-test"
        )
        assert (
            llm["attributes"][AgentInsightOtelSpanAttributes.OBSERVATION_TYPE]
            == "generation"
        )

        # Parse metadata
        proc_metadata = proc["attributes"][
            f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.step"
        ]
        assert proc_metadata == "processing"

        # Parse input/output JSON
        llm_input = json.loads(
            llm["attributes"][AgentInsightOtelSpanAttributes.OBSERVATION_INPUT]
        )
        llm_output = json.loads(
            llm["attributes"][AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT]
        )
        assert llm_input == {"prompt": "Summarize this text"}
        assert llm_output == {"text": "This is a summary"}

    def test_update_current_generation_name(self, agentinsight_client, memory_exporter):
        ...
        with agentinsight_client.start_as_current_observation(
            ...
            agentinsight_client.update_current_generation(
                name="updated-current-generation"
            )
    ...
    def test_start_observation(self, agentinsight_client, memory_exporter):
        ...
        for obs_type, obs_instance in created_observations:
            if obs_type == "span":
                from agentinsight._client.span import AgentInsightSpan

                assert isinstance(obs_instance, AgentInsightSpan), (
                    f"Expected AgentInsightSpan, got {type(obs_instance)}"
                )
            elif obs_type == "generation":
                from agentinsight._client.span import AgentInsightGeneration

                assert isinstance(obs_instance, AgentInsightGeneration), (
                    f"Expected AgentInsightGeneration, got {type(obs_instance)}"
                )
            elif obs_type == "agent":
                from agentinsight._client.span import AgentInsightAgent

                assert isinstance(obs_instance, AgentInsightAgent), (
                    f"Expected AgentInsightAgent, got {type(obs_instance)}"
                )
            elif obs_type == "tool":
                from agentinsight._client.span import AgentInsightTool

                assert isinstance(obs_instance, AgentInsightTool), (
                    f"Expected AgentInsightTool, got {type(obs_instance)}"
                )
            elif obs_type == "chain":
                from agentinsight._client.span import AgentInsightChain

                assert isinstance(obs_instance, AgentInsightChain), (
                    f"Expected AgentInsightChain, got {type(obs_instance)}"
                )
            elif obs_type == "retriever":
                from agentinsight._client.span import AgentInsightRetriever

                assert isinstance(obs_instance, AgentInsightRetriever), (
                    f"Expected AgentInsightRetriever, got {type(obs_instance)}"
                )
            elif obs_type == "evaluator":
                from agentinsight._client.span import AgentInsightEvaluator

                assert isinstance(obs_instance, AgentInsightEvaluator), (
                    f"Expected AgentInsightEvaluator, got {type(obs_instance)}"
                )
            elif obs_type == "embedding":
                from agentinsight._client.span import AgentInsightEmbedding

                assert isinstance(obs_instance, AgentInsightEmbedding), (
                    f"Expected AgentInsightEmbedding, got {type(obs_instance)}"
                )
            elif obs_type == "guardrail":
                from agentinsight._client.span import AgentInsightGuardrail

                assert isinstance(obs_instance, AgentInsightGuardrail), (
                    f"Expected AgentInsightGuardrail, got {type(obs_instance)}"
                )
            elif obs_type == "event":
                from agentinsight._client.span import AgentInsightEvent

                assert isinstance(obs_instance, AgentInsightEvent), (
                    f"Expected AgentInsightEvent, got {type(obs_instance)}"
                )

    def test_custom_trace_id(self, agentinsight_client, memory_exporter):
        ...
        assert span_data["attributes"][AgentInsightOtelSpanAttributes.AS_ROOT] is True

        # Test additional spans with the same trace context
        child_span = agentinsight_client.start_observation(
    ...
    def test_custom_parent_span_id(self, agentinsight_client, memory_exporter):
        ...
        assert spans[0]["attributes"][AgentInsightOtelSpanAttributes.AS_ROOT] is True

    def test_multiple_generations_in_trace(self, agentinsight_client, memory_exporter):
        ...
        assert (
            gen1_data["attributes"][AgentInsightOtelSpanAttributes.OBSERVATION_TYPE]
            == "generation"
        )
        assert (
            gen1_data["attributes"][AgentInsightOtelSpanAttributes.OBSERVATION_MODEL]
            == "gpt-3.5-turbo"
        )

        assert (
            gen2_data["attributes"][AgentInsightOtelSpanAttributes.OBSERVATION_TYPE]
            == "generation"
        )
        assert (
            gen2_data["attributes"][AgentInsightOtelSpanAttributes.OBSERVATION_MODEL]
            == "gpt-4"
        )
    ...
    def test_error_handling(self, agentinsight_client, memory_exporter):
        """Test error handling in span operations."""
        # Create a span that will have an error
        span = agentinsight_client.start_observation(name="error-span")
        ...
        assert attributes[AgentInsightOtelSpanAttributes.OBSERVATION_LEVEL] == "ERROR"
        assert (
            attributes[AgentInsightOtelSpanAttributes.OBSERVATION_STATUS_MESSAGE]
            == "Test error message"
        )

    def test_error_level_in_span_creation(self, agentinsight_client, memory_exporter):
    ...
    def test_error_level_in_span_update(self, agentinsight_client, memory_exporter):
        """Test that OTEL span status is set to ERROR when updating spans to level='ERROR'."""
        # Create a normal span
        span = agentinsight_client.start_observation(
            name="update-error-span", level="INFO"
        )
    ...
    def test_generation_error_level_in_update(
        self, agentinsight_client, memory_exporter
    ):
    ...
    def test_non_error_levels_dont_set_otel_status(
        ...
        for i, level in enumerate(test_levels):
            ...
            span = agentinsight_client.start_observation(name=span_name, level=level)
    ...
    def test_multiple_error_updates(self, agentinsight_client, memory_exporter):
        """Test that multiple ERROR level updates work correctly."""
        # Create a span
        span = agentinsight_client.start_observation(name="multi-error-span")
    ...
    def test_error_without_status_message(self, agentinsight_client, memory_exporter):
...
class TestAdvancedSpans(TestOTelBase):
    ...
    def test_complex_model_parameters(self, agentinsight_client, memory_exporter):
        ...
        model_params = self.verify_json_attribute(
            span_data, AgentInsightOtelSpanAttributes.OBSERVATION_MODEL_PARAMETERS
        )
    ...
    def test_updating_current_generation(self, agentinsight_client, memory_exporter):
        ...
        output = self.verify_json_attribute(
            span_data, AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT
        )
        ...
        if (
            ...
            usage = self.verify_json_attribute(
                span_data, AgentInsightOtelSpanAttributes.OBSERVATION_USAGE_DETAILS
            )
    ...
    def test_shutdown_and_flush(self, agentinsight_client, memory_exporter):
        """Test shutdown and flush operations."""
        # Create a span without ending it
        span = agentinsight_client.start_observation(name="flush-test-span")
        ...
        agentinsight_client.start_observation(name="shutdown-test-span")

        # Call shutdown (should flush any pending spans)
        agentinsight_client.shutdown()
    ...
    def test_trace_id_generation(self, agentinsight_client):
        """Test trace ID generation follows expected format."""
        # Generate trace IDs
        trace_id1 = agentinsight_client.create_trace_id()
        trace_id2 = agentinsight_client.create_trace_id()
...
class TestMetadataHandling(TestOTelBase):
    ...
    def test_complex_metadata_serialization(self):
        ...
        assert AgentInsightOtelSpanAttributes.OBSERVATION_METADATA in non_dict_result
        ...
        assert (
            f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.key1"
            in simple_result
        )
        assert (
            f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.key2"
            in simple_result
        )
        assert (
            simple_result[f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.key1"]
            == "value1"
        )
        assert (
            simple_result[f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.key2"]
            == 123
        )
        ...
        outer_key = f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.outer"
        list_key = f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.list_key"
        ...
        assert AgentInsightOtelSpanAttributes.OBSERVATION_METADATA in none_result
        assert none_result[AgentInsightOtelSpanAttributes.OBSERVATION_METADATA] is None
        ...
        level1_key = f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.level1"
        sibling_key = f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.sibling"
    ...
    def test_nested_metadata_updates(self):
        ...
        from agentinsight._client.attributes import _flatten_and_serialize_metadata
        ...
        config_key = f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.config"
        telemetry_key = (
            f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.telemetry"
        )
        ...
        # Get expected keys
        first_section_key = (
            f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.first_section"
        )
        second_section_key = (
            f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.second_section"
        )
    ...
    def test_thread_safe_metadata_updates(self):
        ...
        user_key = f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.user"
        system_key = f"{AgentInsightOtelSpanAttributes.OBSERVATION_METADATA}.system"
...
class TestMultiProjectSetup(TestOTelBase):
    ...
    def multi_project_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(AgentInsightResourceManager, "_instances", {})
        ...
        original_initialize = AgentInsightResourceManager._initialize_instance
        ...
        def mock_initialize(self, **kwargs):
            ...
            if kwargs.get("public_key") == project1_key:
                ...
                self._otel_tracer = tracer_provider_project2.get_tracer(
                    f"agentinsight:{project2_key}", "test"
                )

        monkeypatch.setattr(
            AgentInsightResourceManager, "_initialize_instance", mock_initialize
        )

        # Initialize the two clients
        langfuse_project1 = AgentInsight(
            public_key=project1_key, secret_key="secret1", base_url="http://test-host"
        )

        langfuse_project2 = AgentInsight(
        ...
        monkeypatch.setattr(
            AgentInsightResourceManager, "_initialize_instance", original_initialize
        )
...
class TestInstrumentationScopeFiltering(TestOTelBase):
    ...
    def instrumentation_filtering_setup(self, monkeypatch):
        ...
        # Mock the AgentInsightSpanProcessor to use our test exporters
        def mock_processor_init(self, **kwargs):
            from opentelemetry.sdk.trace.export import BatchSpanProcessor
        ...
        # We'll add the AgentInsightSpanProcessor to this provider after it's created
        ...
        def mock_initialize(self, **kwargs):
            ...
            # Now create our custom AgentInsightSpanProcessor with the actual blocked_instrumentation_scopes
    ...
    def test_default_filter_exports_current_spans(
    ...
    def test_should_export_span_exception_drops_span_and_logs_error(
        ...
        caplog.set_level("ERROR", logger="agentinsight")
    ...
    def test_blocked_scope_drop_logs_scope_name(
        ...
        caplog.set_level("DEBUG", logger="agentinsight")
...
class TestConcurrencyAndAsync(TestOTelBase):
    ...
    @pytest.mark.asyncio
    async def test_async_span_operations(self, agentinsight_client, memory_exporter):
        """Test async operations with spans."""
        import asyncio

        # Start a main span
        main_span = agentinsight_client.start_observation(name="async-main-span")
        ...
        for i in range(3):
            ...
            output = self.verify_json_attribute(
                task_span, AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT
            )
            assert output["result"] == f"Task {i} completed"

        # Verify main span output
        main_output = self.verify_json_attribute(
            main, AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT
        )
        assert main_output["completed_tasks"] == [0, 1, 2]

    def test_context_propagation_async(self, agentinsight_client, memory_exporter):
        ...
        def thread2_function():
            ...
            thread2_span = agentinsight_client.start_observation(
        ...
        def thread3_function():
            ...
            thread3_span = agentinsight_client.start_observation(
                name="thread3-span",
        ...
        assert (
            thread2_span["attributes"][AgentInsightOtelSpanAttributes.AS_ROOT] is True
    ...
    async def test_span_metadata_updates_in_async_context(
        ...
        if not hasattr(agentinsight_client, "start_observation"):
    ...
    def test_metrics_and_timing(self, agentinsight_client, memory_exporter):
        ...
        # Create a span
        span = agentinsight_client.start_observation(name="timing-test-span")
...
class TestMediaHandling(TestOTelBase):
    ...
    def test_media_with_masking(self):
        ...
        def mask_sensitive_data(data):
            ...
            if isinstance(data, dict):
                ...
                for k, v in data.items():
                    ...
                    elif isinstance(v, AgentInsightMedia):
        ...
        sample_bytes = b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x01\x00H\x00H\x00\x00"
        media = AgentInsightMedia(content_bytes=sample_bytes, content_type="image/jpeg")
    ...
    def test_media_from_file(self):
        ...
        media_from_file = AgentInsightMedia(
            file_path=file_path, content_type="image/jpeg"
        )
        ...
        invalid_media = AgentInsightMedia(
    ...
    def test_masking(self):
        """Test the masking functionality of AgentInsight."""
        ...
        # Now test using the actual AgentInsightSpan implementation
        ...
        # Create a concrete AgentInsightSpan instance
        mock_span = MagicMock()
        span = AgentInsightSpan(otel_span=mock_span, client=mock_client)
...
class TestOtelIdGeneration(TestOTelBase):
    ...
    @pytest.fixture
    def agentinsight_client(self, monkeypatch):
    ...
    def test_trace_id_without_seed(self, agentinsight_client, monkeypatch):
    ...
    def test_trace_id_with_seed(self, agentinsight_client):
        ...
        trace_id_repeat = agentinsight_client.create_trace_id(seed=seed)
        assert trace_id == trace_id_repeat

        # Verify a different seed produces a different ID
        different_seed = "different-identifier"
        different_trace_id = agentinsight_client.create_trace_id(seed=different_seed)
        assert trace_id != different_trace_id

    def test_observation_id_without_seed(self, agentinsight_client, monkeypatch):
        ...
        observation_id = agentinsight_client._create_observation_id()
        assert observation_id == "1234567890abcdef"
        assert len(observation_id) == 16  # 8 bytes hex-encoded = 16 characters

    def test_observation_id_with_seed(self, agentinsight_client):
        """Test observation_id generation with seed (should be deterministic)."""
        seed = "test-identifier"
        observation_id = agentinsight_client._create_observation_id(seed=seed)
        ...
        # Verify the same seed produces the same ID
        observation_id_repeat = agentinsight_client._create_observation_id(seed=seed)
        assert observation_id == observation_id_repeat

        # Verify a different seed produces a different ID
        different_seed = "different-identifier"
        different_observation_id = agentinsight_client._create_observation_id(
            seed=different_seed
        )
        assert observation_id != different_observation_id

    def test_id_generation_consistency(self, agentinsight_client):
        ...
        trace_ids = [agentinsight_client.create_trace_id(seed=seed) for _ in range(5)]
        observation_ids = [
            agentinsight_client._create_observation_id(seed=seed) for _ in range(5)
        ]
    ...
    def test_different_seeds_produce_different_ids(self, agentinsight_client):
        """Test that different seeds produce different IDs."""
        seeds = [f"test-seed-{i}" for i in range(10)]

        # Generate IDs with different seeds
        trace_ids = [agentinsight_client.create_trace_id(seed=seed) for seed in seeds]
        observation_ids = [
            agentinsight_client._create_observation_id(seed=seed) for seed in seeds
        ]
    ...
    def test_langfuse_event_update_immutability(self, agentinsight_client, caplog):
        """Test that AgentInsightEvent.update() logs a warning and does nothing."""
        import logging

        parent_span = agentinsight_client.start_observation(name="parent-span")
        ...
        with caplog.at_level(logging.WARNING, logger="agentinsight._client.span"):
            result = event.update(
                name="updated_name",
                input={"updated": "input"},
            ...
            assert "Attempted to update AgentInsightEvent observation" in caplog.text
