...
from agentinsight._client.attributes import AgentInsightOtelSpanAttributes, _serialize
from agentinsight._client.client import AgentInsight
from agentinsight._client.constants import AGENTINSIGHT_SDK_EXPERIMENT_ENVIRONMENT
from agentinsight._client.datasets import DatasetClient
from agentinsight.api import Dataset, DatasetItem, DatasetStatus
...
class TestPropagateAttributesBase(TestOTelBase):
    ...
    @pytest.fixture
    def agentinsight_client(self, monkeypatch, tracer_provider, mock_processor_init):
        """Create a mocked Langfuse client with explicit tracer_provider for testing."""

        # Set environment variables
        monkeypatch.setenv("AGENTINSIGHT_PUBLIC_KEY", "test-public-key")
        monkeypatch.setenv("AGENTINSIGHT_SECRET_KEY", "test-secret-key")
...
class TestPropagateAttributesBasic(TestPropagateAttributesBase):
    ...
    def test_user_id_propagates_to_child_spans(
        self, agentinsight_client, memory_exporter
    ):
        """Verify user_id propagates to all child spans within context."""
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            with propagate_attributes(user_id="test_user_123"):
                child1 = agentinsight_client.start_observation(name="child-span-1")
                child1.end()

                child2 = agentinsight_client.start_observation(name="child-span-2")
                child2.end()
        ...
        self.verify_span_attribute(
            child1_span,
            AgentInsightOtelSpanAttributes.TRACE_USER_ID,
            "test_user_123",
        )
    ...
    def test_session_id_propagates_to_child_spans(
        ...
        self.verify_span_attribute(
            child1_span,
            AgentInsightOtelSpanAttributes.TRACE_SESSION_ID,
            "session_abc",
        )
    ...
    def test_metadata_propagates_to_child_spans(
        ...
        self.verify_span_attribute(
            child1_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.experiment",
            "variant_a",
        )
        self.verify_span_attribute(
            child1_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.version",
            "1.0",
        )
    ...
    def test_all_attributes_propagate_together(
        ...
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            with propagate_attributes(
                ...
                child = agentinsight_client.start_observation(name="child-span")
                child.end()

        # Verify child has all attributes
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_SESSION_ID, "session_abc"
        )
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.experiment",
        ...
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.env",
            "prod",
        )
...
class TestPropagateAttributesHierarchy(TestPropagateAttributesBase):
    ...
    def test_propagation_to_direct_children(self, agentinsight_client, memory_exporter):
        """Verify attributes propagate to all direct children."""
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            with propagate_attributes(user_id="user_123"):
                child1 = agentinsight_client.start_observation(name="child-1")
                child1.end()

                child2 = agentinsight_client.start_observation(name="child-2")
                child2.end()

                child3 = agentinsight_client.start_observation(name="child-3")
                child3.end()
    ...
    def test_propagation_to_grandchildren(self, agentinsight_client, memory_exporter):
        ...
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            with propagate_attributes(user_id="user_123", session_id="session_abc"):
                with agentinsight_client.start_as_current_observation(
                    name="child-span"
                ):
                    grandchild = agentinsight_client.start_observation(
        ...
        for span in [parent_span, child_span, grandchild_span]:
            self.verify_span_attribute(
                span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
            )
            self.verify_span_attribute(
                span, AgentInsightOtelSpanAttributes.TRACE_SESSION_ID, "session_abc"
            )
    ...
    def test_propagation_across_observation_types(
        ...
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            with propagate_attributes(user_id="user_123"):
                # Create span
                span = agentinsight_client.start_observation(name="test-span")
                span.end()

                # Create generation
                generation = agentinsight_client.start_observation(
        ...
        self.verify_span_attribute(
            span_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )

        generation_data = self.get_span_by_name(memory_exporter, "test-generation")
        self.verify_span_attribute(
            generation_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )
...
class TestPropagateAttributesTiming(TestPropagateAttributesBase):
    ...
    def test_late_propagation_only_future_spans_covered(
        ...
        self.verify_missing_attribute(
            child1_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID
        )

        child2_span = self.get_span_by_name(memory_exporter, "child-2")
        self.verify_span_attribute(
            child2_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )

    def test_current_span_gets_attributes(self, agentinsight_client, memory_exporter):
        ...
        parent_span = self.get_span_by_name(memory_exporter, "parent-span")
        self.verify_span_attribute(
            parent_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )

    def test_spans_outside_context_unaffected(
        self, agentinsight_client, memory_exporter
    ):
        """Verify spans created outside context don't get attributes."""
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            # Span before context
            span1 = agentinsight_client.start_observation(name="span-1")
            span1.end()

            # Span inside context
            with propagate_attributes(user_id="user_123"):
                span2 = agentinsight_client.start_observation(name="span-2")
                span2.end()

            # Span after context
            span3 = agentinsight_client.start_observation(name="span-3")
            span3.end()

        # Verify: only span2 has user_id
        span1_data = self.get_span_by_name(memory_exporter, "span-1")
        self.verify_missing_attribute(
            span1_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID
        )

        span2_data = self.get_span_by_name(memory_exporter, "span-2")
        self.verify_span_attribute(
            span2_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )

        span3_data = self.get_span_by_name(memory_exporter, "span-3")
        self.verify_missing_attribute(
            span3_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID
        )
...
class TestPropagateAttributesValidation(TestPropagateAttributesBase):
    ...
    def test_user_id_over_200_chars_dropped(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_missing_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID
        )
    ...
    def test_session_id_over_200_chars_dropped(
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_missing_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_SESSION_ID
        )
    ...
    def test_metadata_value_over_200_chars_dropped(
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_missing_attribute(
            child_span, f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.key"
        )

    def test_exactly_200_chars_accepted(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, user_id_200
        )

    def test_201_chars_rejected(self, agentinsight_client, memory_exporter):
    ...
    def test_non_string_user_id_dropped(self, agentinsight_client, memory_exporter):
    ...
    def test_mixed_valid_invalid_metadata(self, agentinsight_client, memory_exporter):
        ...
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.valid_key",
            "valid_value",
        )
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.another_valid",
            "ok",
        )
        self.verify_missing_attribute(
            child_span, f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.invalid_key"
        )
...
class TestPropagateAttributesNesting(TestPropagateAttributesBase):
    ...
    def test_nested_contexts_inner_overwrites(
        self, agentinsight_client, memory_exporter
    ):
        ...
        span1_data = self.get_span_by_name(memory_exporter, "span-1")
        self.verify_span_attribute(
            span1_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user1"
        )

        span2_data = self.get_span_by_name(memory_exporter, "span-2")
        self.verify_span_attribute(
            span2_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user2"
        )
    ...
    def test_after_inner_context_outer_restored(
        ...
        span3_data = self.get_span_by_name(memory_exporter, "span-3")
        self.verify_span_attribute(
            span3_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user1"
        )

    def test_nested_different_attributes(self, agentinsight_client, memory_exporter):
        ...
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            with propagate_attributes(user_id="user1"):
                ...
                with propagate_attributes(session_id="session1"):
                    span = agentinsight_client.start_observation(name="span-1")
                    span.end()

        # Verify: span has BOTH user_id and session_id
        span_data = self.get_span_by_name(memory_exporter, "span-1")
        self.verify_span_attribute(
            span_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user1"
        )
        self.verify_span_attribute(
            span_data, AgentInsightOtelSpanAttributes.TRACE_SESSION_ID, "session1"
        )
    ...
    def test_nested_metadata_merges_additively(
        ...
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            with propagate_attributes(metadata={"env": "prod", "region": "us-east"}):
                ...
                outer_span = agentinsight_client.start_observation(name="outer-span")
                outer_span.end()
                ...
                with propagate_attributes(
                    ...
                    inner_span = agentinsight_client.start_observation(
                        name="inner-span"
                    )
                    inner_span.end()

                # Back to outer context
                after_span = agentinsight_client.start_observation(name="after-span")
                after_span.end()
        ...
        self.verify_span_attribute(
            outer_span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.region",
            "us-east",
        )
        self.verify_missing_attribute(
            outer_span_data,
    ...
    def test_triple_nested_metadata_accumulates(
            ...
            with propagate_attributes(metadata={"level": "1", "a": "outer"}):
                with propagate_attributes(metadata={"level": "2", "b": "middle"}):
                    with propagate_attributes(metadata={"level": "3", "c": "inner"}):
                        span = agentinsight_client.start_observation(name="deep-span")
                        span.end()
        ...
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.level",
            "3",
        )
        ...
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.a",
            "outer",
        )
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.b",
            "middle",
        )
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.c",
            "inner",
        )

    def test_metadata_merge_with_empty_inner(
        self, agentinsight_client, memory_exporter
    ):
        ...
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.key1",
            "value1",
        )
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.key2",
            "value2",
        )
    ...
    def test_metadata_merge_preserves_user_session(
        ...
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.outer",
            "value",
        )
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.inner",
            "value",
        )
...
class TestPropagateAttributesEdgeCases(TestPropagateAttributesBase):
    ...
    def test_none_values_ignored(self, agentinsight_client, memory_exporter):
    ...
    def test_empty_metadata_dict(self, agentinsight_client, memory_exporter):
    ...
    def test_all_invalid_metadata_values(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_missing_attribute(
            child_span, f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.key1"
        )
        self.verify_missing_attribute(
            child_span, f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.key2"
        )

    def test_propagate_with_no_active_span(self, agentinsight_client, memory_exporter):
        """Verify propagate_attributes works even with no active span."""
        # Call propagate_attributes without creating a parent span first
        with propagate_attributes(user_id="user_123"):
            # Now create a span
            with agentinsight_client.start_as_current_observation(name="span-1"):
                pass
...
class TestPropagateAttributesFormat(TestPropagateAttributesBase):
    ...
    def test_user_id_uses_correct_attribute_name(
        ...
        assert AgentInsightOtelSpanAttributes.TRACE_USER_ID in child_span["attributes"]
        assert (
            child_span["attributes"][AgentInsightOtelSpanAttributes.TRACE_USER_ID]
            == "user_123"
        )
    ...
    def test_session_id_uses_correct_attribute_name(
        ...
        # Verify the exact attribute key is used
        assert (
            AgentInsightOtelSpanAttributes.TRACE_SESSION_ID in child_span["attributes"]
        )
        assert (
            child_span["attributes"][AgentInsightOtelSpanAttributes.TRACE_SESSION_ID]
            == "session_abc"
        )

    def test_metadata_keys_properly_prefixed(
        self, agentinsight_client, memory_exporter
    ):
    ...
    def test_multiple_metadata_keys_independent(
        ...
        assert attributes[f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.k1"] == "v1"
        assert attributes[f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.k2"] == "v2"
        assert attributes[f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.k3"] == "v3"
...
class TestPropagateAttributesThreading(TestPropagateAttributesBase):
    ...
    def test_propagation_with_threadpoolexecutor(
        ...
        def worker_function(span_name: str):
            ...
            span = agentinsight_client.start_observation(name=span_name)
            span.end()
            return span_name

        with agentinsight_client.start_as_current_observation(name="main-span"):
    ...
    def test_propagation_isolated_between_threads(
        ...
        def create_trace_with_user(user_id: str):
            ...
            with agentinsight_client.start_as_current_observation(
                ...
                with propagate_attributes(user_id=user_id):
                    span = agentinsight_client.start_observation(name=f"span-{user_id}")
                    span.end()
        ...
        self.verify_span_attribute(
            span1, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user1"
        )

        span2 = self.get_span_by_name(memory_exporter, "span-user2")
        self.verify_span_attribute(
            span2, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user2"
        )
    ...
    def test_nested_propagation_across_thread_boundary(
        ...
        def worker_creates_child():
            ...
            child = agentinsight_client.start_observation(name="worker-child-span")
            child.end()

        with agentinsight_client.start_as_current_observation(name="main-parent-span"):
            with propagate_attributes(user_id="main_user"):
                # Create span in main thread
                main_child = agentinsight_client.start_observation(
                    name="main-child-span"
                )
                main_child.end()
        ...
        self.verify_span_attribute(
            main_child_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "main_user"
        )

        worker_child_span = self.get_span_by_name(memory_exporter, "worker-child-span")
        self.verify_span_attribute(
            worker_child_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "main_user"
        )
    ...
    def test_worker_thread_can_override_propagated_attrs(
        ...
        def worker_overrides_user():
            ...
            with propagate_attributes(user_id="worker_user"):
                span = agentinsight_client.start_observation(name="worker-span")
                span.end()

        with agentinsight_client.start_as_current_observation(name="main-span"):
            with propagate_attributes(user_id="main_user"):
                # Create span in main thread
                main_span = agentinsight_client.start_observation(
                    name="main-child-span"
                )
                main_span.end()
        ...
        self.verify_span_attribute(
            main_child, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "main_user"
        )

        worker_span = self.get_span_by_name(memory_exporter, "worker-span")
        self.verify_span_attribute(
            worker_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "worker_user"
        )
    ...
    def test_multiple_workers_with_same_propagated_context(
        ...
        def worker_function(worker_id: int):
            ...
            span = agentinsight_client.start_observation(name=f"worker-{worker_id}")
            span.end()
    ...
    def test_concurrent_traces_with_different_attributes(
        ...
        def create_trace(trace_id: int):
            ...
            with agentinsight_client.start_as_current_observation(
                ...
                with propagate_attributes(user_id=f"user_{trace_id}"):
                    span = agentinsight_client.start_observation(
                        name=f"span-{trace_id}"
                    )
                    span.end()
        ...
        for i in range(10):
            ...
            self.verify_span_attribute(
                span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, f"user_{i}"
            )
    ...
    def test_exception_in_worker_preserves_context(
        ...
        with agentinsight_client.start_as_current_observation(name="main-span"):
            with propagate_attributes(user_id="main_user"):
                ...
                span1 = agentinsight_client.start_observation(name="span-before")
                span1.end()
                ...
                span2 = agentinsight_client.start_observation(name="span-after")
                span2.end()

        # Verify both main thread spans still have correct user_id
        span_before = self.get_span_by_name(memory_exporter, "span-before")
        self.verify_span_attribute(
            span_before, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "main_user"
        )

        span_after = self.get_span_by_name(memory_exporter, "span-after")
        self.verify_span_attribute(
            span_after, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "main_user"
        )
...
class TestPropagateAttributesCrossTracer(TestPropagateAttributesBase):
    ...
    def test_different_tracer_spans_get_attributes(
        self, agentinsight_client, memory_exporter, tracer_provider
    ):
        ...
        with agentinsight_client.start_as_current_observation(name="langfuse-parent"):
            with propagate_attributes(user_id="user_123", session_id="session_abc"):
                # Create span with Langfuse tracer
                current_span = agentinsight_client.start_observation(
                    name="langfuse-child"
                )
                current_span.end()
        ...
        current_span_data = self.get_span_by_name(memory_exporter, "langfuse-child")
        self.verify_span_attribute(
            current_span_data,
            AgentInsightOtelSpanAttributes.TRACE_USER_ID,
            "user_123",
    ...
    def test_nested_spans_from_multiple_tracers(
        ...
        with agentinsight_client.start_as_current_observation(name="root"):
            with propagate_attributes(
                ...
                with tracer_a.start_as_current_span(name="library-a-span"):
                    with tracer_b.start_as_current_span(name="library-b-span"):
                        langfuse_leaf = agentinsight_client.start_observation(
                            name="langfuse-leaf"
                        )
                        langfuse_leaf.end()
    ...
    def test_other_tracer_span_before_propagate_context(
        ...
        span_before = self.get_span_by_name(memory_exporter, "span-before")
        self.verify_missing_attribute(
            span_before, AgentInsightOtelSpanAttributes.TRACE_USER_ID
        )

        span_after = self.get_span_by_name(memory_exporter, "span-after")
        self.verify_span_attribute(
            span_after, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )
    ...
    def test_mixed_tracers_with_metadata(
        ...
        with agentinsight_client.start_as_current_observation(name="main"):
            with propagate_attributes(
                metadata={
                    "env": "production",
                    "version": "2.0",
        ...
        for span_name in ["langfuse-operation", "library-operation"]:
            ...
            self.verify_span_attribute(
                span_data,
                f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.feature_flag",
                "enabled",
            )
    ...
    def test_propagate_without_langfuse_parent(
        ...
        with other_tracer.start_as_current_span(name="other-parent"):
            with propagate_attributes(user_id="user_123", session_id="session_xyz"):
                ...
                langfuse_child = agentinsight_client.start_observation(
                    name="langfuse-child"
                )
                langfuse_child.end()
    ...
    def test_attributes_persist_across_tracer_changes(
        ...
        with agentinsight_client.start_as_current_observation(name="root"):
            with propagate_attributes(user_id="persistent_user"):
                ...
                current_span = agentinsight_client.start_observation(name="step-4")
                current_span.end()
...
class TestPropagateAttributesAsync(TestPropagateAttributesBase):
    ...
    @pytest.mark.asyncio
    async def test_async_propagation_basic(self, agentinsight_client, memory_exporter):
        """Verify attributes propagate in async context."""

        async def async_operation():
            """Async function that creates a span."""
            span = agentinsight_client.start_observation(name="async-span")
            span.end()

        with agentinsight_client.start_as_current_observation(name="parent"):
        ...
        self.verify_span_attribute(
            async_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "async_user"
        )
        self.verify_span_attribute(
            async_span, AgentInsightOtelSpanAttributes.TRACE_SESSION_ID, "async_session"
        )

    @pytest.mark.asyncio
    async def test_async_nested_operations(self, agentinsight_client, memory_exporter):
        """Verify attributes propagate through nested async operations."""

        async def level_3():
            span = agentinsight_client.start_observation(name="level-3-span")
            span.end()

        async def level_2():
            span = agentinsight_client.start_observation(name="level-2-span")
            span.end()
            await level_3()

        async def level_1():
            span = agentinsight_client.start_observation(name="level-1-span")
            span.end()
            await level_2()
        ...
        for span_name in ["level-1-span", "level-2-span", "level-3-span"]:
            span_data = self.get_span_by_name(memory_exporter, span_name)
            self.verify_span_attribute(
                span_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "nested_user"
            )
            self.verify_span_attribute(
                span_data,
                f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.level",
    ...
    @pytest.mark.asyncio
    async def test_async_context_manager(self, agentinsight_client, memory_exporter):
        ...
        with agentinsight_client.start_as_current_observation(name="parent"):
            ...
            with propagate_attributes(user_id="async_ctx_user"):
                span = agentinsight_client.start_observation(name="inside-async-ctx")
                span.end()

        span_data = self.get_span_by_name(memory_exporter, "inside-async-ctx")
        self.verify_span_attribute(
            span_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "async_ctx_user"
        )
    ...
    async def test_multiple_async_tasks_concurrent(
        ...
        for user_id in ["user1", "user2", "user3"]:
            ...
            self.verify_span_attribute(
                span_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, user_id
            )

    @pytest.mark.asyncio
    async def test_async_with_sync_nested(self, agentinsight_client, memory_exporter):
        """Verify attributes propagate from async to sync code."""

        def sync_operation():
            """Sync function called from async context."""
            span = agentinsight_client.start_observation(name="sync-in-async")
            span.end()

        async def async_operation():
            """Async function that calls sync code."""
            span1 = agentinsight_client.start_observation(name="async-span")
            span1.end()
            sync_operation()
        ...
        async_span = self.get_span_by_name(memory_exporter, "async-span")
        self.verify_span_attribute(
            async_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "mixed_user"
        )

        sync_span = self.get_span_by_name(memory_exporter, "sync-in-async")
        self.verify_span_attribute(
            sync_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "mixed_user"
        )
    ...
    async def test_async_exception_preserves_context(
        ...
        async def failing_operation():
            ...
            span = agentinsight_client.start_observation(name="span-before-error")
            span.end()
            raise ValueError("Test error")

        with agentinsight_client.start_as_current_observation(name="root"):
            with propagate_attributes(user_id="error_user"):
                span1 = agentinsight_client.start_observation(name="span-before-async")
                span1.end()
                ...
                span2 = agentinsight_client.start_observation(name="span-after-error")
                span2.end()
        ...
        for span_name in ["span-before-async", "span-before-error", "span-after-error"]:
            span_data = self.get_span_by_name(memory_exporter, span_name)
            self.verify_span_attribute(
                span_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "error_user"
            )

    @pytest.mark.asyncio
    async def test_async_with_metadata(self, agentinsight_client, memory_exporter):
        """Verify metadata propagates correctly in async context."""

        async def async_with_metadata():
            span = agentinsight_client.start_observation(name="async-metadata-span")
            span.end()
        ...
        self.verify_span_attribute(
            span_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "metadata_user"
        )
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.async",
            "true",
        )
        self.verify_span_attribute(
            span_data,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.operation",
            "test",
        )
...
class TestPropagateAttributesBaggage(TestPropagateAttributesBase):
    ...
    def test_baggage_is_set_when_as_baggage_true(self, agentinsight_client):
    ...
    def test_spans_receive_attributes_from_baggage(
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "baggage_user"
        )
        self.verify_span_attribute(
            child_span,
            AgentInsightOtelSpanAttributes.TRACE_SESSION_ID,
        ...
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.source",
            "baggage",
        )

    def test_baggage_disabled_by_default(self, agentinsight_client):
    ...
    def test_metadata_key_with_user_id_substring_doesnt_collide(
        ...
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.user_info",
            "some_data",
        )
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.user_id_copy",
            "another",
        )
    ...
    def test_metadata_key_with_session_substring_doesnt_collide(
        ...
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.session_data",
            "value1",
        )
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.session_id_backup",
            "value2",
        )
    ...
    def test_metadata_keys_extract_correctly_from_baggage(
        ...
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.experiment_id",
            "exp_123",
        )
    ...
    def test_baggage_and_context_both_propagate(
        ...
        with agentinsight_client.start_as_current_observation(name="parent"):
            ...
            with propagate_attributes(
                ...
                with agentinsight_client.start_as_current_observation(name="middle"):
                    child = agentinsight_client.start_observation(name="leaf")
                    child.end()
        ...
        for span_name in ["parent", "middle", "leaf"]:
            span_data = self.get_span_by_name(memory_exporter, span_name)
            self.verify_span_attribute(
                span_data, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_both"
            )
            self.verify_span_attribute(
                span_data,
                AgentInsightOtelSpanAttributes.TRACE_SESSION_ID,
    ...
    def test_baggage_survives_context_isolation(
        ...
        with agentinsight_client.start_as_current_observation(name="original-process"):
            with propagate_attributes(
        ...
        try:
            with agentinsight_client.start_as_current_observation(
                ...
                child = agentinsight_client.start_observation(name="remote-child")
                child.end()
        finally:
...
class TestPropagateAttributesVersion(TestPropagateAttributesBase):
    ...
    def test_version_propagates_to_child_spans(
        ...
        self.verify_span_attribute(
            child1_span,
            AgentInsightOtelSpanAttributes.VERSION,
            "v1.2.3",
        )
    ...
    def test_version_with_user_and_session(self, agentinsight_client, memory_exporter):
        ...
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.VERSION, "2.0.0"
        )

    def test_version_with_metadata(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.VERSION, "1.0.0"
        )
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.env",
    ...
    def test_version_validation_over_200_chars(
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_missing_attribute(
            child_span, AgentInsightOtelSpanAttributes.VERSION
        )

    def test_version_exactly_200_chars(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.VERSION, version_200
        )
    ...
    def test_version_nested_contexts_inner_overwrites(
        ...
        span1_data = self.get_span_by_name(memory_exporter, "span-1")
        self.verify_span_attribute(
            span1_data, AgentInsightOtelSpanAttributes.VERSION, "1.0.0"
        )

        span2_data = self.get_span_by_name(memory_exporter, "span-2")
        self.verify_span_attribute(
            span2_data, AgentInsightOtelSpanAttributes.VERSION, "2.0.0"
        )

        span3_data = self.get_span_by_name(memory_exporter, "span-3")
        self.verify_span_attribute(
            span3_data, AgentInsightOtelSpanAttributes.VERSION, "1.0.0"
        )

    def test_version_with_baggage(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.VERSION, "baggage_version"
        )
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )
    ...
    def test_version_semantic_versioning_formats(
        ...
        with agentinsight_client.start_as_current_observation(name="parent-span"):
            for idx, version in enumerate(test_versions):
                with propagate_attributes(version=version):
                    span = agentinsight_client.start_observation(name=f"span-{idx}")
                    span.end()
        ...
        for idx, expected_version in enumerate(test_versions):
            ...
            self.verify_span_attribute(
                span_data, AgentInsightOtelSpanAttributes.VERSION, expected_version
            )

    def test_version_non_string_dropped(self, agentinsight_client, memory_exporter):
    ...
    def test_version_propagates_to_grandchildren(
        ...
        for span in [parent_span, child_span, grandchild_span]:
            self.verify_span_attribute(
                span, AgentInsightOtelSpanAttributes.VERSION, "nested_v1"
            )

    @pytest.mark.asyncio
    async def test_version_with_async(self, agentinsight_client, memory_exporter):
        ...
        async_span = self.get_span_by_name(memory_exporter, "async-span")
        self.verify_span_attribute(
            async_span, AgentInsightOtelSpanAttributes.VERSION, "async_v1.0"
        )

    def test_version_attribute_key_format(self, agentinsight_client, memory_exporter):
        ...
        assert AgentInsightOtelSpanAttributes.VERSION in attributes
        assert attributes[AgentInsightOtelSpanAttributes.VERSION] == "key_test_v1"


class TestPropagateAttributesTags(TestPropagateAttributesBase):
    """Tests for tags parameter propagation."""

    def test_tags_propagate_to_child_spans(self, agentinsight_client, memory_exporter):
        ...
        self.verify_span_attribute(
            child1_span,
            AgentInsightOtelSpanAttributes.TRACE_TAGS,
    ...
    def test_tags_with_single_tag(self, agentinsight_client, memory_exporter):
    ...
    def test_empty_tags_list(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_missing_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_TAGS
        )

    def test_tags_with_user_and_session(self, agentinsight_client, memory_exporter):
    ...
    def test_tags_with_metadata(self, agentinsight_client, memory_exporter):
    ...
    def test_tags_validation_with_invalid_tag(
        self, agentinsight_client, memory_exporter
    ):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_TAGS, tuple(["valid_tag"])
        )
    ...
    def test_tags_with_baggage(self, agentinsight_client, memory_exporter):
    ...
    def test_tags_propagate_to_grandchildren(
        self, agentinsight_client, memory_exporter
    ):
    ...
    @pytest.mark.asyncio
    async def test_tags_with_async(self, agentinsight_client, memory_exporter):
    ...
    def test_tags_attribute_key_format(self, agentinsight_client, memory_exporter):
        ...
        # Verify exact attribute key
        assert AgentInsightOtelSpanAttributes.TRACE_TAGS in attributes
        assert attributes[AgentInsightOtelSpanAttributes.TRACE_TAGS] == tuple(
            ["key_test"]
        )
...
class TestPropagateAttributesExperiment(TestPropagateAttributesBase):
    ...
    async def test_experiment_propagates_user_id_in_async_context(
        ...
        with propagate_attributes(user_id="async-experiment-user"):
            agentinsight_client.run_experiment(
                name="Async Experiment",
                data=local_data,
                task=async_task,
            )

        agentinsight_client.flush()
    ...
    def test_experiment_attributes_propagate_without_dataset(
        ...
        result = agentinsight_client.run_experiment(
        ...
        self.verify_span_attribute(
            first_root,
            AgentInsightOtelSpanAttributes.EXPERIMENT_DESCRIPTION,
            "Test experiment description",
        )
        self.verify_span_attribute(
            first_root,
            AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_EXPECTED_OUTPUT,
            _serialize("expected result 1"),
        )

        # Propagated attributes should also be on root
        experiment_id = first_root["attributes"][
            AgentInsightOtelSpanAttributes.EXPERIMENT_ID
        ]
        assert result.experiment_id == experiment_id
        experiment_item_id = first_root["attributes"][
            AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_ID
        ]
        root_observation_id = first_root["attributes"][
            AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_ROOT_OBSERVATION_ID
        ]

        self.verify_span_attribute(
            first_root,
            AgentInsightOtelSpanAttributes.EXPERIMENT_NAME,
            result.run_name,
        )
        for metadata_key, metadata_value in experiment_metadata.items():
            self.verify_span_attribute(
                first_root,
                f"{AgentInsightOtelSpanAttributes.EXPERIMENT_METADATA}.{metadata_key}",
                metadata_value,
            )

        self.verify_span_attribute(
            first_root,
            f"{AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_METADATA}.item_type",
            "test",
        )
        self.verify_span_attribute(
            first_root,
            f"{AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_METADATA}.priority",
            "high",
        )
        ...
        self.verify_span_attribute(
            first_root,
            AgentInsightOtelSpanAttributes.ENVIRONMENT,
            AGENTINSIGHT_SDK_EXPERIMENT_ENVIRONMENT,
        )

        # Dataset ID should not be set for local data
        self.verify_missing_attribute(
            first_root,
            AgentInsightOtelSpanAttributes.EXPERIMENT_DATASET_ID,
        )
    ...
    def test_experiment_id_is_stable_across_local_items(
        ...
        root_spans = self.get_spans_by_name(memory_exporter, "experiment-item-run")
        experiment_ids = {
            span["attributes"][AgentInsightOtelSpanAttributes.EXPERIMENT_ID]
    ...
    def test_experiment_attributes_propagate_with_dataset(
        self, agentinsight_client, memory_exporter, monkeypatch
    ):
        ...
        def mock_create_dataset_run_item(*args, **kwargs):
            from agentinsight.api import DatasetRunItem
        ...
        monkeypatch.setattr(
            agentinsight_client.api.dataset_run_items,
            "create",
        ...
        dataset = DatasetClient(
            ...
            agentinsight_client=agentinsight_client,
        )

        # Task with child spans
        def task_with_children(*, item, **kwargs):
            child1 = agentinsight_client.start_observation(name="dataset-child-1")
            child1.end()

            child2 = agentinsight_client.start_observation(name="dataset-child-2")
            child2.end()
        ...
        for child_span in child_spans[:2]:
            ...
            self.verify_span_attribute(
                child_span,
                f"{AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_METADATA}.source",
                "dataset",
            )
            self.verify_span_attribute(
                child_span,
                f"{AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_METADATA}.index",
                "0",
            )
    ...
    def test_experiment_attributes_propagate_to_nested_children(
        ...
        def task_with_nested_spans(*, item, **kwargs):
            with agentinsight_client.start_as_current_observation(name="child-span"):
                with agentinsight_client.start_as_current_observation(
                    name="grandchild-span"
                ):
                    great_grandchild = agentinsight_client.start_observation(
    ...
    def test_experiment_metadata_merging(self, agentinsight_client, memory_exporter):
        """Test that experiment metadata and item metadata are both propagated correctly."""
        from agentinsight._client.attributes import _serialize
        ...
        def task_with_child(*, item, **kwargs):
            child = agentinsight_client.start_observation(name="metadata-child")
            child.end()
            return "result"
        ...
        for metadata_key, metadata_value in item_metadata.items():
            self.verify_span_attribute(
                child_span,
                f"{AgentInsightOtelSpanAttributes.EXPERIMENT_ITEM_METADATA}.{metadata_key}",
                _serialize(metadata_value),
            )
    ...
    def test_experiment_metadata_values_are_validated_individually(
        self, agentinsight_client, memory_exporter, caplog
    ):
        """Experiment metadata is flattened so large combined dicts still propagate."""

        caplog.set_level("WARNING", logger="agentinsight")
        ...
        def task_with_child(*, item, **kwargs):
            child = agentinsight_client.start_observation(name="large-metadata-child")
            child.end()
            return "result"
        ...
        self.verify_missing_attribute(
            child_span,
            AgentInsightOtelSpanAttributes.EXPERIMENT_METADATA,
        )
...
class TestPropagateAttributesTraceName(TestPropagateAttributesBase):
    ...
    def test_trace_name_propagates_to_child_spans(
        ...
        self.verify_span_attribute(
            child1_span,
            AgentInsightOtelSpanAttributes.TRACE_NAME,
            "my-trace-name",
        )
    ...
    def test_trace_name_propagates_to_grandchildren(
        ...
        for span in [parent_span, child_span, grandchild_span]:
            self.verify_span_attribute(
                span, AgentInsightOtelSpanAttributes.TRACE_NAME, "nested-trace"
            )

    def test_trace_name_with_user_and_session(
        self, agentinsight_client, memory_exporter
    ):
        ...
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_NAME, "combined-trace"
        )

    def test_trace_name_with_version(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_NAME, "versioned-trace"
        )
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.VERSION, "1.0.0"
        )

    def test_trace_name_with_metadata(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_NAME, "metadata-trace"
        )
        self.verify_span_attribute(
            child_span,
            f"{AgentInsightOtelSpanAttributes.TRACE_METADATA}.env",
    ...
    def test_trace_name_validation_over_200_chars(
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_missing_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_NAME
        )

    def test_trace_name_exactly_200_chars(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_NAME, trace_name_200
        )
    ...
    def test_trace_name_nested_contexts_inner_overwrites(
        ...
        span1_data = self.get_span_by_name(memory_exporter, "span-1")
        self.verify_span_attribute(
            span1_data, AgentInsightOtelSpanAttributes.TRACE_NAME, "outer-trace"
        )

        span2_data = self.get_span_by_name(memory_exporter, "span-2")
        self.verify_span_attribute(
            span2_data, AgentInsightOtelSpanAttributes.TRACE_NAME, "inner-trace"
        )

        span3_data = self.get_span_by_name(memory_exporter, "span-3")
        self.verify_span_attribute(
            span3_data, AgentInsightOtelSpanAttributes.TRACE_NAME, "outer-trace"
        )

    def test_trace_name_sets_on_current_span(
        self, agentinsight_client, memory_exporter
    ):
        ...
        parent_span = self.get_span_by_name(memory_exporter, "parent-span")
        self.verify_span_attribute(
            parent_span, AgentInsightOtelSpanAttributes.TRACE_NAME, "current-trace"
        )

    def test_trace_name_non_string_dropped(self, agentinsight_client, memory_exporter):
    ...
    def test_trace_name_with_baggage(self, agentinsight_client, memory_exporter):
        ...
        child_span = self.get_span_by_name(memory_exporter, "child-span")
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_NAME, "baggage-trace"
        )
        self.verify_span_attribute(
            child_span, AgentInsightOtelSpanAttributes.TRACE_USER_ID, "user_123"
        )
