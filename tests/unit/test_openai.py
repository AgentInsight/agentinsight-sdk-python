...
import agentinsight.openai as lf_openai_module
from agentinsight._client.attributes import AgentInsightOtelSpanAttributes
from agentinsight.openai import openai as lf_openai
...
def test_chat_completion_exports_generation_span(
    ...
    assert (
        span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_TYPE] == "generation"
    )
    assert (
    ...
    assert json_attr(span, AgentInsightOtelSpanAttributes.OBSERVATION_INPUT) == [
        {"role": "user", "content": "1 + 1 = ?"}
    ]
    assert json_attr(span, AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT) == {
        "role": "assistant",
        "content": "2",
    }
    assert json_attr(
        span, AgentInsightOtelSpanAttributes.OBSERVATION_MODEL_PARAMETERS
    ) == {
        "temperature": 0,
        "max_tokens": "Infinity",
        "top_p": 1,
    ...
    assert json_attr(
        span, AgentInsightOtelSpanAttributes.OBSERVATION_USAGE_DETAILS
    ) == {
        "prompt_tokens": 3,
        "completion_tokens": 1,
        "total_tokens": 4,
...
def test_streaming_chat_completion_exports_ttft(
    ...
    assert span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT] == "2"
    assert (
        span.attributes[
...
def test_chat_completion_error_marks_generation_error(langfuse_memory_client, get_span):
    ...
    assert AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT not in span.attributes
...
def test_fallback_sync_stream_finalizes_once():
    ...
    wrapper = lf_openai_module.AgentInsightResponseGeneratorSync(
        resource=resource,
...
async def test_fallback_async_stream_finalizes_once():
    ...
    wrapper = lf_openai_module.AgentInsightResponseGeneratorAsync(
        resource=resource,
        response=fallback_stream(),
        generation=generation,
    )
...
def test_embedding_exports_dimensions_and_count(
    ...
    assert (
        span.attributes[AgentInsightOtelSpanAttributes.OBSERVATION_TYPE] == "embedding"
    )
    assert json_attr(span, AgentInsightOtelSpanAttributes.OBSERVATION_OUTPUT) == {
        "dimensions": 3,
        "count": 1,
